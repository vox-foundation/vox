...
- Lints unchanged or stricter. **Exceptions:** `vox-tensor` uses `#![cfg_attr(feature = "gpu", allow(missing_docs))]` (Burn wrappers). `vox-populi` may use `#![allow(missing_docs)]` on the Mens/training surface (CLI/training wiring). Do not copy these to compiler/runtime/core crates.
