...
Vox embraces a server-first web architecture. In Vox v0.3+, the old monolithic `@island` decorator has been replaced by the `@island` component system alongside raw programmatic HTTP routing. 

## Interactive Islands

Client-side interactive user interfaces are modeled using hydrated React components known as islands. 

- `@island fn ComponentName(props: ModelType) to Element { }`  
...
- `http post "/path" (body: BodyType) to ResultType { }`  
...
routes {
    "/"              to HomeIsland         
    "/dashboard"     to AccountDashboardIsland
    "/terms"         to StaticTermsContent
}
```
