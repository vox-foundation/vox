...
- **`--features dashboard`**: reserved **no-op** in `vox-cli`. The old **`vox mens` chat / agent / dei / learn** commands are removed from the CLI surface (they depended on the historical **`vox-dei`** module tree, not the minimal workspace crate). Use **`vox-codex-dashboard`** / the VS Code extension for dashboard-style surfaces.
