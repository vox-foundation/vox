...
**Optional NVML-backed GPU inventory** on join/heartbeat `NodeRecord`s (ADR 018 Layer A): add **`mesh-nvml-probe`** (e.g. `cargo build -p vox-cli --features populi,mesh-nvml-probe`). Requires NVIDIA driver/NVML at runtime; see [GPU truth probe spec](../architecture/populi-gpu-truth-probe-spec.md).
...
**Boundaries:** Vox does **not** ship a shell emulator product. See [Vox shell operations boundaries](../architecture/vox-shell-operations-boundaries.md).
...
Optional IDE wiring: [`.vscode/settings.json`](../../../.vscode/settings.json) adds terminal profiles **Vox Exec policy (PSReadLine)** (loads [`.agents/workflows/vox_interceptor_profile.ps1`](../../../.agents/workflows/vox_interceptor_profile.ps1)) and **Vox pwsh proxy (check only)** ([`.vox/bin/vox-pwsh-proxy.cmd`](../../../.vox/bin/vox-pwsh-proxy.cmd) — set `VOX_SHELL_CHECK_PAYLOAD` to the line to validate). See also [`terminal-ast-validation-research-2026.md`](../architecture/terminal-ast-validation-research-2026.md).
