...
| `VOX_ORCHESTRATOR_MESH_CONTROL_URL` | Base URL of the mesh HTTP control plane for **read-only** node snapshots in MCP/orchestrator (e.g. `http://mesh-ctrl:9847`). See [mesh SSOT](../architecture/mesh-ssot.md), [deployment compose SSOT](../architecture/deployment-compose-ssot.md). |
...
| `VOX_ORCHESTRATOR_MESH_ROUTING_EXPERIMENTAL` | Experimental routing hooks (see [mesh SSOT](../architecture/mesh-ssot.md)). |
...
| `VOX_INFERENCE_PROFILE` | `desktop_ollama` (default), `cloud_openai_compatible`, `mobile_litert`, `mobile_coreml`, `lan_gateway`; gates **vox-mcp** local Ollama + Ollama fallback to `desktop_ollama` / `lan_gateway` only; see [`vox_config::inference`](../../../crates/vox-config/src/inference.rs) and [mobile-edge-ai-ssot.md](../architecture/mobile-edge-ai-ssot.md). |

## Mesh (`vox-mesh`, orchestrator probe)

Full table: [mesh SSOT](../architecture/mesh-ssot.md). Common entries:
...
| `VOX_MESH_HTTP_JOIN` | `0`/`false` disables MCP HTTP join to the control plane; see [mesh SSOT](../architecture/mesh-ssot.md). |
...
| `VOX_EMIT_EXPRESS_SERVER` | Opt-in: emit legacy **`server.ts`** (Express-style) from `vox-codegen-ts`; default product is **Axum** + **`api.ts`**. See [vox-fullstack-artifacts-ssot.md](../architecture/vox-fullstack-artifacts-ssot.md). |
...
- [Deployment compose SSOT](../architecture/deployment-compose-ssot.md) — Compose profiles and Coolify/GitLab notes.
