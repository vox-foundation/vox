...
> self-terminating training job design for `vox schola train`. All preset VRAM thresholds, 
...
### Winner for Mens Training: **Vast.ai + `--onstart` hook**
...
vastai create instance <offer_id> \
  ...
  --onstart-cmd 'bash -c "
    pip install -q huggingface_hub vox-mens &&
    vox schola train --model $VOX_MODEL --preset auto &&
    huggingface-cli upload my-org/vox-mens mens/runs/latest &&
    vastai stop self
  "'
```
...
```bash
gcloud compute instances create vox-schola-$(date +%s) \
  ...
  --metadata=startup-script='
    #!/bin/bash
    cd /workspace
    vox schola train --model Qwen/Qwen2.5-Coder-3B-Instruct --preset auto
    gsutil -m cp -r mens/runs/latest gs://my-bucket/vox-mens/
...
For native integration, `vox schola train --cloud vast|runpod|gcp` should:
...
vox schola train ... --output-dir /workspace/mens/runs/v1
```
