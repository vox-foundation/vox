...
| `VOX_MESH_TOKEN` | When set on **`vox populi serve`**, all HTTP routes except **`GET /health`** require `Authorization: Bearer <token>`. Clients use **`PopuliHttpClient::with_env_token`**. **Never log** this value. |
...
For in-process tests or custom hosts, **`populi_http_app_with_auth`** + **`PopuliHttpAuth`** (`Open`, `Bearer(…)`, or `FromEnv`) avoid relying on ambient `VOX_MESH_TOKEN` in the test process.
