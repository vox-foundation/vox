---
title: "Populi native training SSOT (Burn LoRA, Candle qlora-rs QLoRA)"
description: "Official documentation for Populi native training SSOT (Burn LoRA, Candle qlora-rs QLoRA) for the Vox language."
category: "reference"
...
3. **Training acceptance profile**: `cargo run -p vox-cli -- ci populi-gate --profile training` (see [populi-finetune-acceptance-runbook.md](../architecture/populi-finetune-acceptance-runbook.md)).
...
- **Thin shims** (delegate to the same command): `bash scripts/populi/release_training_gate.sh`, `pwsh scripts/populi/release_training_gate.ps1`  
  Mirrors [`populi-finetune-acceptance-runbook.md`](../architecture/populi-finetune-acceptance-runbook.md) rows 1–10 (planner, keymap, strict preflight, Burn smoke, parity tests, merge, `merge_v2`).
...
- **LLM / agent PR hygiene:** [`populi-llm-pr-checklist.md`](../architecture/populi-llm-pr-checklist.md) — LoRA duplication, layouts, merge, CI test names, parity tiers.
- **LoRA ownership boundary:** [`populi-lora-ownership.md`](populi-lora-ownership.md)
- **Speech / ASR** (Oratio): [`oratio-speech.md`](oratio-speech.md) — orthogonal to training; shares `vox populi` CLI namespace only. CLI STT commands need **`vox-cli`** feature **`populi-oratio`** (not default **`populi-base`**).
