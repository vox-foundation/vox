---
title: "Mesh Coordination & Database Write Safety"
description: "Official documentation for Mesh Coordination & Database Write Safety for the Vox language. Detailed technical reference, architecture gui"
category: "reference"
last_updated: 2026-03-24
training_eligible: true
---

# Mesh Coordination & Database Write Safety

Single Source of Truth for how Vox mesh nodes coordinate on Turso/libSQL,
...
> persists to Turso when `VOX_MESH_ENABLED=1`. On a single machine without mesh
...
```
┌────────────────────────────────────┐  ┌────────────────────────────────────┐
│       Mesh Node A  (Device 1)      │  │       Mesh Node B  (Device 2)      │
│                                    │  │                                    │
...
Durable inbox for agent-to-agent messages. Cross-node delivery via the mesh HTTP
...
query `live_nodes_from_db(stale_threshold_ms)` to see the full mesh membership.
...
## 4. EmbeddedReplica for Mesh Nodes
...
|----------|---------|---------|
| `VOX_MESH_ENABLED` | `false` | Activate mesh coordination |
...
| `VOX_MESH_TOKEN` | unset | Bearer token for mesh HTTP routes |
...
| NATS JetStream for durable A2A at scale | Not needed at current mesh size | When >100 concurrent agents |
