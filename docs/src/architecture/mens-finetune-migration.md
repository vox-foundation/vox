...
- **`vox schola train`** still uses `--backend lora|qlora`, but validation is **contract-first** inside `vox-mens` (`FineTuneContract`, `ExecutionPlanner`, `preflight_train`).
...
- Prefer **`vox schola train`** over legacy `vox train --native-lora` (already deprecated in CLI messaging).
