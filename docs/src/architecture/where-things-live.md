...
| [`vox-mesh-transport`](../../../crates/vox-mesh-transport/) | iroh QUIC mesh transport: persisted mesh identity, EndpointId trust allowlist, and the job wire protocol. |
...
| Agent browser snapshot + refs / named profiles / Chrome attach | Planned per [`docs/superpowers/specs/2026-09-07-agent-browser-driver-design.md`](../../superpowers/specs/2026-09-07-agent-browser-driver-design.md): `crates/vox-plugin-browser/src/{snapshot,ref_actions,hosts,policy}.rs` + MCP `vox_browser_snapshot` / `*_ref` / `open_ex` in `vox-orchestrator-mcp`; profiles via `vox_config::paths::browser_profiles_dir()` (`$VOX_DATA_DIR/browser-profiles`, user data — not Tier D). Vox `Browser.*` builtins stay CSS-only in v1. |
...
| Add a research-pipeline stage (claims/gate/planner/provider/types/verifier) | `crates/vox-orchestrator/src/orchestrator/task_dispatch/research_dispatch.rs` (autonomous research, multi-wave CRAG loop, triplet grounding) and `crates/vox-orchestrator/src/orchestrator/task_dispatch/`. **`web_gather`** delegates to `vox-search` (`WebSearchDispatcher`, `CragRouter`); planner/claims/verifier remain Phase 0a stubs until `vox-claim-extractor`. CLI: `vox research run`; MCP: `vox_research_run`. **Telemetry bridge / policy feedback / mesh tap:** `crates/vox-research-shim/src/research/{research_event_metrics_bridge.rs, search_policy_feedback.rs, mesh_subscriber.rs}`; narrative [`research-scientia-telemetry-channels.md`](./research-scientia-telemetry-channels.md). |
