...
| Per-script memory ceiling (counting allocator enforcing `--max-memory`) | **Planned — lands in a later PR (mesh-phase3 PR 5).** `crates/vox-cli/src/mem_limit.rs` does not exist yet; this row is added now per the interpreter-first plan's Task 3 so the lookup table doesn't drift behind the plan. |
...
| Isolation / capability model reference doc (grammar, embedder defaults, fs-root canonicalization rules) | **Planned — lands in a later PR (mesh-phase3 PR 5).** `docs/src/reference/isolation.md` does not exist yet; until then, the grammar and embedder table live as doc comments on `CapabilitySet` in `crates/vox-compiler/src/eval/caps.rs`. |
