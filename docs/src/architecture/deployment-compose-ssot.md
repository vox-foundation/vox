...
| **MCP + mesh (multi-service)** | Control plane + MCP + worker; shared registry volume | [`examples/mesh-compose.yml`](../../../examples/mesh-compose.yml) | Same `Dockerfile` with build-arg `VOX_CLI_FEATURES=mesh,script-execution` | **9847** (mesh), **3000** (MCP) |
...
**Do not** assume root `docker-compose.yml` and `infra/coolify/docker-compose.yml` are interchangeable: they target **different workloads** (MCP vs Codex API template). See [Codex BaaS](codex-baas.md) and [infra/coolify/README.md](../../../infra/coolify/README.md).
...
- **Codex / Turso:** `VOX_DB_URL`, `VOX_DB_TOKEN`, `VOX_DB_PATH` — [env-vars SSOT](../reference/env-vars-ssot.md), [ADR 004](../adr/004-codex-arca-turso-ssot.md).
- **Mesh:** full `VOX_MESH_*` table — [mesh SSOT](mesh-ssot.md). Optional **`VOX_ORCHESTRATOR_MESH_CONTROL_URL`** for MCP to read mesh nodes (see [`examples/mesh-compose.yml`](../../../examples/mesh-compose.yml)). With a client-suitable URL, **`vox-mcp`** also **HTTP join/heartbeat** to the control plane (see mesh SSOT **`VOX_MESH_HTTP_*`**).
- **Optional mesh env block (one text SSOT):** [`docker/vox-compose-mesh-environment.block.yaml`](../../../docker/vox-compose-mesh-environment.block.yaml) — embedded into generated Compose in `vox-container`; keep [`examples/mesh-compose.yml`](../../../examples/mesh-compose.yml) semantically aligned (comments in that file point here).
- **Inference / mobile:** `VOX_INFERENCE_PROFILE` and LAN/cloud patterns — [mobile / edge AI SSOT](mobile-edge-ai-ssot.md) (phones do **not** run this `Dockerfile`).
...
- [Cross-platform Vox — lanes & Docker matrix (SSOT)](vox-cross-platform-runbook.md) — script worker vs app vs mobile; feature matrix.
- [How to deploy](../how-to-deploy.md) — `vox deploy`, `Vox.toml`, registry login.
- [Zig-inspired deployment](zig-inspired-deployment.md) — unified `vox deploy` targets and crates.
- [Mesh SSOT](mesh-ssot.md), [orchestration unified SSOT](orchestration-unified-ssot.md).
- [Mobile / edge AI SSOT](mobile-edge-ai-ssot.md).

## Do’s and don’ts (short)

- **Do** keep variable names identical to [env-vars SSOT](../reference/env-vars-ssot.md) / mesh / ADR 004.
