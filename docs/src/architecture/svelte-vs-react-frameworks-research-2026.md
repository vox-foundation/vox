...
|---|---|---|
| `component Name() { }` keyword (per [AGENTS.md §Grammar Unification](../../../AGENTS.md)) | [crates/vox-compiler/src/ast/decl/fundecl.rs](../../../crates/vox-compiler/src/ast/decl/fundecl.rs) | Implemented; lowers to React TSX in [codegen_ts/component.rs](../../../crates/vox-compiler/src/codegen_ts/component.rs) |
| **Reactive component members `state`, `derived`, `effect`, `on mount`, `on cleanup`, `view:`** | AST: [ast/decl/ui.rs:152–238](../../../crates/vox-compiler/src/ast/decl/ui.rs); parser: [parser/descent/decl/head.rs:253–334](../../../crates/vox-compiler/src/parser/descent/decl/head.rs); codegen: [codegen_ts/reactive.rs:740–815](../../../crates/vox-compiler/src/codegen_ts/reactive.rs); golden: [examples/golden/reactive_counter.vox](../../../examples/golden/reactive_counter.vox) | **Implemented inside a `component { }` block.** Lowers to `useState` / `useMemo` / `useEffect`; auto-emits React deps lists |
| Auto-dep inference (compile-time read-tracking for `derived` and `effect`) | [codegen_ts/hir_emit/state_deps.rs](../../../crates/vox-compiler/src/codegen_ts/hir_emit/state_deps.rs) | Partial: AST walk that finds identifiers matching reactive-binding names; descends into `Lambda` bodies; does **not** cross function-decl boundaries (over-tracks within scope, under-tracks across calls) |
| **Directive-style attributes `on:click`, `on:change`, `on:input`, `on:submit`, `on:keydown` …** | Parser: [parser/descent/expr/pratt_jsx.rs:27–33](../../../crates/vox-compiler/src/parser/descent/expr/pratt_jsx.rs); mapping: [codegen_ts/hir_emit/compat.rs:27–42](../../../crates/vox-compiler/src/codegen_ts/hir_emit/compat.rs) | **Implemented for the `on:*` family only.** Both `name:suffix` (colon) and `name-suffix` (hyphen) attribute-name syntax are accepted by the parser |
...
| Web IR (intermediate before TSX) | [crates/vox-compiler/src/web_ir/mod.rs](../../../crates/vox-compiler/src/web_ir/mod.rs) | Implemented; G4 parity tests against legacy emit path |
| Compile-time a11y validators (img alt, button accessible name, keyboard handler on `role="button"`) | [crates/vox-compiler/src/web_ir/validate_a11y.rs](../../../crates/vox-compiler/src/web_ir/validate_a11y.rs) | Implemented; structured diagnostic codes `web_ir_validate.a11y.*` |
| Web IR validators (DOM bounds, route IDs, behavior optionality, island prop keys, broken `href`/`to` against route patterns) | [crates/vox-compiler/src/web_ir/validate.rs](../../../crates/vox-compiler/src/web_ir/validate.rs) | Implemented |
| `routes { }` block, type-safe nested entries with `path` / `component_name` / `loader_name` / `pending_component_name` / `redirect` / wildcard | [crates/vox-compiler/src/ast/decl/ui.rs](../../../crates/vox-compiler/src/ast/decl/ui.rs) + [codegen_ts/route_manifest.rs](../../../crates/vox-compiler/src/codegen_ts/route_manifest.rs) | Implemented; conflict detection is exact-string-match only ([routes.rs:87](../../../crates/vox-compiler/src/codegen_ts/routes.rs)) — does not detect `/users/:id` vs `/users/me` overlap |
| `state_machine` keyword, typed states + transitions, discriminated-union types **and reducer function stub** | [crates/vox-compiler/src/ast/decl/state_machine.rs](../../../crates/vox-compiler/src/ast/decl/state_machine.rs) + [state_machine_emit.rs:1–110](../../../crates/vox-compiler/src/codegen_ts/state_machine_emit.rs) | Implemented as types + reducer skeleton; no instantiable runtime object or live state field |
| Design tokens (JSON → CSS custom properties + typed `voxTokens` constant) | [codegen_ts/tokens_emit.rs](../../../crates/vox-compiler/src/codegen_ts/tokens_emit.rs) | Implemented |
...
| **MCP compile-and-validate tool: `vox_validate_file`** ("Validate a .vox file using the full compiler pipeline") **plus `vox_compiler::ast_inspect`** | Registry: [contracts/mcp/tool-registry.canonical.yaml:1211](../../../contracts/mcp/tool-registry.canonical.yaml); dispatch: [crates/vox-orchestrator/src/mcp_tools/dispatch.rs:237](../../../crates/vox-orchestrator/src/mcp_tools/dispatch.rs); MCP crates: `vox-mcp-meta`, `vox-mcp-registry`; stdio server: [crates/vox-cli/src/commands/mcp_server/mod.rs](../../../crates/vox-cli/src/commands/mcp_server/mod.rs) | Implemented; surfaced through the existing stdio MCP server |
| External-frontend interop pipeline (target split, OpenAPI/JSON Schema emit, bidirectional component interop) | [external-frontend-interop-plan-2026.md](external-frontend-interop-plan-2026.md), [phase5-react-interop-spec-2026.md](phase5-react-interop-spec-2026.md) | Plan / specs; Phase 1 in flight |
...
| Route conflict detection | Exact-string-match only ([routes.rs:87](../../../crates/vox-compiler/src/codegen_ts/routes.rs)) | Segment-aware | Partial | Segment-aware |
| Broken-link compile check | Yes ([web_ir/validate.rs](../../../crates/vox-compiler/src/web_ir/validate.rs)) | Via SvelteKit type-check | No | Via typed router |
...
**What's already done.** `state name: T = init`, `derived name = expr`, `effect: { … }`, `on mount: { … }`, `on cleanup: { … }`, and `view: …` all parse and lower correctly inside a `component { }` block — see [reactive_counter.vox](../../../examples/golden/reactive_counter.vox) and [codegen_ts/reactive.rs:740–815](../../../crates/vox-compiler/src/codegen_ts/reactive.rs). Auto-dep inference for `derived` and `effect` runs via [hir_emit/state_deps.rs](../../../crates/vox-compiler/src/codegen_ts/hir_emit/state_deps.rs) and emits the React deps array automatically.
...
**What's already done.** `on:*` event-handler directives work today, parsed at [pratt_jsx.rs:27–33](../../../crates/vox-compiler/src/parser/descent/expr/pratt_jsx.rs:27) (which already accepts both colon- and hyphen-prefixed attribute names) and mapped to React props at [compat.rs:27–42](../../../crates/vox-compiler/src/codegen_ts/hir_emit/compat.rs:27). The colon separator is the established Vox convention; M3 is *additional families*, not a new syntax.
...
**What's already done.** `vox_validate_file` ("Validate a .vox file using the full compiler pipeline (lexer → parser → typeck → HIR)") is registered in [contracts/mcp/tool-registry.canonical.yaml:1211](../../../contracts/mcp/tool-registry.canonical.yaml) and dispatched at [crates/vox-orchestrator/src/mcp_tools/dispatch.rs:237](../../../crates/vox-orchestrator/src/mcp_tools/dispatch.rs). The companion `vox_compiler::ast_inspect` tool ([same file:351](../../../crates/vox-orchestrator/src/mcp_tools/dispatch.rs)) returns the parsed AST as JSON. The stdio MCP server is wired through [crates/vox-cli/src/commands/mcp_server/mod.rs](../../../crates/vox-cli/src/commands/mcp_server/mod.rs).
...
- **Segment-aware overlap detection.** Current check at [routes.rs:87](../../../crates/vox-compiler/src/codegen_ts/routes.rs) builds a `HashSet<(Method, String)>` and only catches identical literal strings. Extend to detect `/users/:id` vs `/users/me` overlap (with documented precedence resolution).
- **Typed `href` helper.** [validate.rs](../../../crates/vox-compiler/src/web_ir/validate.rs) already validates literal `href` and `to` attributes against declared route patterns (broken-link diagnostic). Extend to a typed helper API so authors can write `href={route.users.show(id)}` and get full type-flow.
...
- [Phase 5: Bidirectional Vox↔React Interop Spec (2026)](phase5-react-interop-spec-2026.md) — the React-interop spec this research is *not* trying to displace.
