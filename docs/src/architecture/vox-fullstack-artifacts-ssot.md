---
title: "Vox full-stack build artifacts (SSOT)"
...
This document names **every major output** of `vox build` / `vox run` / `vox bundle` and the **canonical runtime** for the default product path. It complements [vox-web-stack-ssot.md](vox-web-stack-ssot.md) and [ADR 010 — TanStack web spine](../adr/010-tanstack-web-spine.md).
...
[`vox-codegen-ts`](../../crates/vox-codegen-ts/src/routes.rs) can emit **`server.ts`**, an **Express** app that duplicates `@server` and `http` route registration.

- **Default:** emission is **off** unless **`VOX_EMIT_EXPRESS_SERVER=1`** is set in the environment when running codegen (e.g. `vox build`). The supported client for `@server fn` against Axum is **`api.ts`** from **Rust** codegen ([`emit_api_client`](../../crates/vox-codegen-rust/src/emit.rs)).
...
- [ref-cli.md](../ref-cli.md) — CLI surface.
