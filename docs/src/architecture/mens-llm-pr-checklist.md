...
Use this when agents or humans touch **`vox-mens` training**, **merge commands**, **LoRA/QLoRA**, or **parity tests**. Goal: avoid typical **context-blind** mistakes (wrong crate, wrong layout, doc drift).

## Duplication and ownership

- [ ] **Two `lora.rs` trees:** `crates/vox-tensor/src/lora.rs` (primitives) vs `crates/vox-mens/src/tensor/lora.rs` (transformer + merge). Fixes to **linear LoRA math** may need **both** or a deliberate consolidation. Canonical split: [`mens-lora-ownership.md`](../reference/mens-lora-ownership.md).
...
- [ ] **`cfg(feature = "train")`** on `vox-mens` exports (e.g. `MERGE_QLORA_REJECTS_BURN_BIN`): every binary that needs them must enable `vox-mens/train` (see `vox-cli` `gpu` feature wiring).
...
- [ ] **CI job vs test binary:** `.github/workflows/ci.yml` `--test <name>` must match `crates/vox-mens/tests/<name>.rs`.
