...
| D | M1+M4 — `.vox.ui` modules (reactive members at module scope) | **🟡 Foundation + parser slice shipped** ([954ad8775](https://github.com/vox-foundation/vox), [26c90f9be](https://github.com/vox-foundation/vox)). `vox_compiler::module::FileKind` helper + `parse_with_kind` entry + new `Decl::ReactiveModule` variant + parser dispatch. **HIR lowering + codegen pending** (next slices). | — | **✅ ADR-032 accepted** |
...
| F | M2 — Typed parametric fragment primitive | **🟡 Skeleton shipped** ([6f01b8ae1](https://github.com/vox-foundation/vox)). Lexer `Token::Fragment` + `Decl::Fragment(FragmentDecl)` + parser dispatch — authors can write `fragment Name(args) { … }` and it parses into a typed AST node. **Codegen still gated** on Phase 6 (TASK-6.1) typed-primitive stabilization per ADR-033. | TASK-6.1 + ADR-033 | **✅ ADR-033 accepted** |
| G | M7 — Reactive-class state-machine instances | **🟡 Runtime helper shipped** (`crates/vox-runtime/src/state_machine.rs`). Generic `ReactiveStateMachine<S, E>` with `state()` / `send()` / `Clone`-via-`Arc` semantics; 5 unit tests. **Codegen wiring pending** (will emit `useFooStateMachine` hooks built on this helper). | D | No |
