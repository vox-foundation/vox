...
|---|---|---|---|---|
| A | M5 — `vox_validate_source` + autofix surfacing | **XS (~1–2 days)** — vox_lsp already populates `data: { suggestions, fixes }`; only the MCP-side `DiagnosticInfo` struct needs the field added | — | No |
...
| C | M6 — Route segment-aware overlap + typed `href` | M (~1–1.5 weeks) | — | No |
| D | M1+M4 — `.vox.ui` modules (reactive members at module scope) | M-L (~2 weeks) — **upgraded** because file-extension dispatch is fragmented across CLI commands ([build.rs](../../../crates/vox-cli/src/commands/build.rs), check, dev, etc.); ADR must include the wire-up surface, not just the suffix policy | — | **Yes** (ADR-032) |
| E | M1 — Cross-call auto-dep inference | M (~1.5–2 weeks) | — | No |
