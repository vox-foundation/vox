...
- `on:click`, `on:change`, `on:input`, `on:submit`, `on:keydown` directive-style attributes ship today ([compat.rs:27–42](../../../crates/vox-compiler/src/codegen_ts/hir_emit/compat.rs)). The parser already accepts arbitrary `name:suffix` and `name-suffix` attribute names ([pratt_jsx.rs:27–33](../../../crates/vox-compiler/src/parser/descent/expr/pratt_jsx.rs)).
...
|---|---|---|---|---|
| A | M5 — `vox_validate_source` + autofix surfacing | **✅ Code + docs complete** ([8ae08edab](https://github.com/anthropics/vox)). `DiagnosticInfo.{code,fixes}` plumbed end-to-end; new `vox_validate_source` tool in registry/dispatch/http_gateway. User-facing how-to at [how-to-mcp-vox-validate.md](../how-to/how-to-mcp-vox-validate.md) ([423a44e7e](https://github.com/anthropics/vox)) covers Claude Desktop / Cursor wiring, diagnostic shape, the iterate-against-the-compiler loop, and the diagnostic-code namespaces. | — | No |
...
| C | M6 — Route segment-aware overlap + typed `href` | **✅ Foundation + integration shipped** ([e0dd30fb0](https://github.com/anthropics/vox), [5be659ed1](https://github.com/anthropics/vox)). `RoutePattern::parse` + `Overlap::{None,Shadowed,Ambiguous}` + `validate_express_route_emit_input` upgraded to segment-aware. **✅ Typed `routePath` builder shipped** ([latest](https://github.com/anthropics/vox)). `KnownRoute` union + `routePath["/users/:id"](id)` builder map in `routes.manifest.ts`. **`url`-decl ↔ routes cross-validation deferred** — slice A bypassed the need by generating builders directly from route patterns; explicit `url` decls remain a general-purpose typed-URL construct independent of `routes { }`. | — | No |
| D | M1+M4 — `.vox.ui` modules (reactive members at module scope) | M-L (~2 weeks) — **upgraded** because file-extension dispatch is fragmented across CLI commands ([build.rs](../../../crates/vox-cli/src/commands/build.rs), check, dev, etc.); ADR must include the wire-up surface, not just the suffix policy | — | **Yes** (ADR-032 drafted, awaiting acceptance) |
| E | M1 — Cross-call auto-dep inference + `@reactive` decorator | **✅ Shipped end-to-end** ([5db51fc0a](https://github.com/anthropics/vox), [a1c6f46ec](https://github.com/anthropics/vox), [6691bf831](https://github.com/anthropics/vox)). `@reactive` decorator parses + lowers; analyzer recurses one level into `@reactive` callees with a self-reference guard; reactive emit uses the analyzer; tier-2 `dep_inference.over_track` hint surface emits a `// dep_inference.over_track` comment in TSX naming unannotated callees. | — | No |
| F | M2 — Typed parametric fragment primitive | **Design drafted ([ADR-033](../adr/033-typed-fragment-primitive.md)); code deferred** until Phase-6 primitives stabilize (avoids designing fragments against a deprecated authoring surface) | TASK-6.1 + ADR-033 | Yes (drafted, awaiting acceptance) |
| G | M7 — Reactive-class state-machine instances | M (~1.5 weeks); helpers land in `vox-runtime` (no `vox-stdlib` exists) | D | No |

**Shipped this session (2026-05-02 → 2026-05-03):** Phases A, C foundation + integration + typed-builder, E foundation + wiring + tier-2. Plus housekeeping: ADR-032 drafted ([0fcb7340d](https://github.com/anthropics/vox), [4df880e7c](https://github.com/anthropics/vox), [e45629d8f](https://github.com/anthropics/vox), [277942a63](https://github.com/anthropics/vox), [f0a3d1b75](https://github.com/anthropics/vox)) — orphan-snapshot cleanup, two stale-test fixes (ADR-028 / TASK-2.6 alignment), inbound-link goldens fix, `view_roots`-setup gap fix, contributor doc note on snapshot-baseline workflow. Full vox-compiler test suite went from 7 known failures → 0.
