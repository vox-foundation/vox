...
| TASK-4.2 — Add effect annotations (`uses net, db, mcp(...)`) | P0 ("function secretly touches network" unrepresentable via effect propagation), P3 (effects declared locally on the signature; no global flow analysis), P2 (`uses` clause distinctive) | ❌ Not started | Preconditions: Phase 2 complete. ~2–3 weeks. |
...
**Phase 4 verdict:** 2 complete (TASK-4.3, TASK-4.4), 0 partial, 2 not started.
...
2. **TASK-4.2** — Add effect annotations (`uses net, db, mcp(...)`) (P0: secret network access unrepresentable).
3. **TASK-0.6** — Harden `transport.ts`: verify backoff caps and `authStatus` event emission.
4. **TASK-1.2** — Decide on `vox-dashboard-d` binary (Option A: delete, Option B: make it work).
