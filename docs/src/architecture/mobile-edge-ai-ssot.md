---
title: "Mobile / edge AI — training, inference, mesh (SSOT)"
...
This page is the **single place** for how Vox treats **Android / iOS / browser** relative to desktop Populi training, **Ollama**, **mesh** coordination, and **GPU** advertisement. It complements [Populi training SSOT](populi-training-ssot.md), [mesh SSOT](mesh-ssot.md), and [unified orchestration](orchestration-unified-ssot.md).
...
3. **Artifacts** (`adapter_schema_v3`, `training_manifest.json`) record `training_deployment_target` and an operator **note** pointing here and to [HF finetune capability matrix](hf-finetune-capability-matrix.md). **Conversion** to LiteRT / Core ML / TFLite is **out of tree** until a supported exporter exists.

Canonical trainer documentation remains [populi-training-ssot.md](populi-training-ssot.md).
...
Further context: [HF finetune capability matrix](hf-finetune-capability-matrix.md), [Populi training SSOT](populi-training-ssot.md).
...
Registry: [Environment variables (SSOT)](../reference/env-vars-ssot.md) (`VOX_INFERENCE_PROFILE`).
...
See [mesh SSOT](mesh-ssot.md) for the full `VOX_MESH_*` table.
...
- [Cross-platform Vox — lanes & Docker matrix (SSOT)](vox-cross-platform-runbook.md) — script worker vs app vs mobile; Docker feature matrix.
- [Deployment compose SSOT](deployment-compose-ssot.md) — server/container Compose vs mobile (inference profiles, no phone OCI).
- [Orchestration unified SSOT](orchestration-unified-ssot.md) — capability merge rules.
