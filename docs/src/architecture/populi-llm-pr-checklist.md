...
- [ ] **`cfg(feature = "train")`** on `vox-populi` exports (e.g. `MERGE_QLORA_REJECTS_BURN_BIN`): every binary that needs them must enable `vox-populi/train` (see `vox-cli` `gpu` feature wiring).
...
- [ ] **CI job names vs runbook:** `.github/workflows/ci.yml` Populi steps should stay aligned with [`populi-finetune-acceptance-runbook.md`](populi-finetune-acceptance-runbook.md) (same `cargo test` filters, e.g. `execution_planner` not multiple filters on one line).
...
- [ ] **CI job vs test binary:** `.github/workflows/ci.yml` `--test <name>` must match `crates/vox-populi/tests/<name>.rs`.
