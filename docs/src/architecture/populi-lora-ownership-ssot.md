...
- PRs touching both trees: use [`populi-llm-pr-checklist.md`](populi-llm-pr-checklist.md) and add/adjust a **regression test** in the kernel that actually runs the changed path (`vox-populi` train or merge tests; `vox-tensor` unit tests for primitives).

## Related

- [`populi-training-ssot.md`](populi-training-ssot.md) — CLI, kernels, manifests, CI commands.
- [`hf-finetune-capability-matrix.md`](hf-finetune-capability-matrix.md) — supported combos.
