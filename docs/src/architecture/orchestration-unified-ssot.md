---
title: "Unified orchestration (compatibility + v2 rollout)"
...
See also [mesh SSOT](./mesh-ssot.md) for `VOX_MESH_*` and local registry.
...
| `VOX_MESH_MAX_STALE_MS` | Client-side filter for mesh node lists in MCP snapshots (see [mesh SSOT](./mesh-ssot.md)). |
...
Optional **`[mesh]`** in `Vox.toml` merges mesh scope/URL/labels for CLI and MCP (see [mesh SSOT](./mesh-ssot.md)); **env wins** per field when set.
...
- [`external-repositories-ssot.md`](./external-repositories-ssot.md) — `repository_id`, sessions, cache layout.
- [`socrates-protocol-ssot.md`](./socrates-protocol-ssot.md) — Socrates telemetry and policy.
- [`populi-training-ssot.md`](./populi-training-ssot.md) — training backends and env.
