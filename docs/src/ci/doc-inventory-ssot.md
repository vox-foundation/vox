...
**Canonical CI entrypoint:** **`vox ci …`** (GitHub Actions often uses `cargo run -p vox-cli --quiet -- ci …` before `vox` is on `PATH`). See [Runner contract](runner-contract.md) (section *Canonical `vox ci` vs shell scripts*).
