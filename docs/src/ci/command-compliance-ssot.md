---
title: "Command compliance (registry + CI gate)"
category: ci
...
| Doc needles for `ref_cli_required` operations | `docs/src/ref-cli.md` — `vox ci …` and `vox codex` subcommands are checked **only inside** their `### \`vox ci …\`` / `### \`vox codex\`` sections (not whole-file substring matches) |
| Top-level reachability table rows | `docs/src/architecture/cli-reachability-ssot.md` (rows skipped for `completions`, `fabrica`, `mens`, `ars`, `recensio`, and when `reachability_required: false`) |
...
**CI:** `.github/workflows/ci.yml` runs this gate after **`vox ci check-docs-ssot`** (after **`vox ci line-endings`** and other early guards; see [workflow enumeration](workflow-enumeration.md)).

**Definition of done** for a new shipped CLI operation: registry row + docs + **`command-compliance`** green (see [`cli-design-rules-ssot.md`](../architecture/cli-design-rules-ssot.md)).
