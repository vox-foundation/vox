---
title: "How To: Model Routing & Provider Cascade"
category: how-to
constructs: []
...
| **Efficient / free-only** | `VOX_DEI_MODE_PROFILE=efficient` or MCP `mode_profile: efficient` keeps `free_only` routing; OpenRouter defaults stay on free/auto when the usage tracker runs with `free_only`. See [efficient-mode.md](how-to/efficient-mode.md). |
