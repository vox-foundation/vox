...
Separately, **`vox-dei-d`** remains the RPC process for Mens-shaped AI flows (`ai.generate`, `ai.review`, `ai.plan.*`) with stable method ids in `vox-cli` `dei_daemon.rs`. It is **not** defined as the host for the full `Orchestrator` type today.
...
2. **`vox-dei-d` boundary:** Keep **`vox-dei-d`** focused on **DeI RPC / AI routing** unless an explicit future ADR moves **`Orchestrator`** behind a dedicated **`vox-orchestrator-d`** (or merged daemon) with MCP behaving as a thin client — out of scope for this ADR.
...
4. Unchanged: **`vox-dei-d`** stays on the **AI plane** until an explicit merge ADR.
...
- [`crates/vox-orchestrator/src/orchestrator_event_log.rs`](../../../crates/vox-orchestrator/src/orchestrator_event_log.rs) — `VOX_ORCHESTRATOR_EVENT_LOG` JSONL sink
