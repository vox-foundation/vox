...
Vox needs a **CPU-first** mesh: workers advertise capabilities and can federate beyond a single process. We want one control-plane stack to avoid dual maintenance (no parallel gRPC + QUIC servers in-tree).
...
- Operators run `vox mesh serve` behind `nginx`/`caddy`/`Envoy` for TLS and auth.
