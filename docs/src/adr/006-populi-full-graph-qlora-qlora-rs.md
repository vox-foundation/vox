...
5. **Adapter format v2 + merge**  
   Adapters export LoRA matrices per logical layer (`mid0`, …, `lm_head`) with sidecar JSON mapping adapter prefixes → base safetensors keys. `vox populi merge-qlora` merges LoRA deltas into **f32** base tensors for those keys (reload for inference outside this ADR).
