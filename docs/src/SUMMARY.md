# Summary


# Getting Started

- [Vox: The AI-Native Programming Language](api/decorators/index.md)

# Tutorials

- [Getting Started with Vox](tutorials/tut-getting-started.md)
- [Tutorial: Actor Basics](tutorials/tut-actor-basics.md)
- [Tutorial: Building a Collaborative Todo List](tutorials/tut-first-app.md)
- [Tutorial: UI Integration](tutorials/tut-ui-integration.md)
- [Tutorial: Workflow Durability](tutorials/tut-workflow-durability.md)

# How-To Guides

- [@py.import – Python Library Integration (`torch`, `numpy`, etc.)](how-to/how-to-pytorch.md)
- [Ecosystem & Tooling](how-to/how-to-cli-ecosystem.md)
- [Examples](how-to/examples-corpus.md)
- [Examples corpus & strict parse](how-to/examples-corpus.md)
- [First full-stack Vox app](how-to/first-full-stack-app.md)
- [How To: Train Populi on RTX 4080 Super](how-to/how-to-train-populi-4080.md)
- [How-To: Build AI Agents and MCP Tools](how-to/how-to-ai-agents.md)
- [How-To: Deploy to Production](how-to/how-to-deploy.md)
- [How-To: Handle Errors Gracefully](how-to/how-to-error-handling.md)
- [How-To: Model Complex Domain Logic](how-to/how-to-custom-types.md)
- [How-To: Test Your Logic](how-to/how-to-unit-test.md)
- [Model Routing & Provider Cascade](how-to/how-to-model-routing.md)
- [Setup & Installation](how-to/how-to-setup.md)
- [TanStack SSR with Axum (development topology)](how-to/tanstack-ssr-with-axum.md)

# Explanations

- [Actors & Workflows](explanation/expl-actors-workflows.md)
- [Compiler Architecture](explanation/expl-architecture.md)
- [Explanation: Compiler Lowering Phases](explanation/expl-compiler-lowering.md)
- [Explanation: Durable Execution](explanation/expl-durable-execution.md)
- [Explanation: Security Model](explanation/expl-security.md)
- [Explanation: The Vox Runtime](explanation/expl-runtime.md)
- [Glossary: Vox Terminology](explanation/glossary.md)
- [MCP Optimization Strategy](explanation/expl-mcp-optimizations.md)
- [Native ML Training Pipeline](explanation/expl-ml-pipeline.md)
- [OpenClaw Competitive Analysis](explanation/expl-openclaw-analysis.md)
- [Research Notes: Achieving Serverless-like Performance with MCP](explanation/mcp_serverless_research.md)
- [Vox FAQ: Frequently Asked Questions](explanation/faq.md)
- [Zig-Inspired Deployment Architecture](explanation/zig-inspired-deployment.md)

# Reference

- [@action](api/decorators/action.md)
- [@component](api/decorators/component.md)
- [@deprecated](api/decorators/deprecated.md)
- [@index](api/decorators/index.md)
- [@mcp.tool](api/decorators/mcp_tool.md)
- [@mutation](api/decorators/mutation.md)
- [@pure](api/decorators/pure.md)
- [@query](api/decorators/query.md)
- [@require](api/decorators/require.md)
- [@scheduled](api/decorators/scheduled.md)
- [@server](api/decorators/server.md)
- [@table](api/decorators/table.md)
- [@test](api/decorators/test.md)
- [@v0](api/decorators/v0.md)
- [actor](api/keywords/actor.md)
- [await](api/keywords/await.md)
- [Changelog](reference/changelog.md)
- [cli](reference/cli.md)
- [Codex HTTP API](reference/codex-http-api.md)
- [Command compliance](reference/command-compliance.md)
- [Crate API: vox-ast](api/vox-ast.md)
- [Crate API: vox-codegen-rust](api/vox-codegen-rust.md)
- [Crate API: vox-codegen-ts](api/vox-codegen-ts.md)
- [Crate API: vox-config](api/vox-config.md)
- [Crate API: vox-db](api/vox-db.md)
- [Crate API: vox-doc-pipeline](api/vox-doc-pipeline.md)
- [Crate API: vox-gamify](api/vox-gamify.md)
- [Crate API: vox-hir](api/vox-hir.md)
- [Crate API: vox-integration-tests](api/vox-integration-tests.md)
- [Crate API: vox-lexer](api/vox-lexer.md)
- [Crate API: vox-lsp](api/vox-lsp.md)
- [Crate API: vox-mcp](api/vox-mcp.md)
- [Crate API: vox-orchestrator](api/vox-orchestrator.md)
- [Crate API: vox-parser](api/vox-parser.md)
- [Crate API: vox-pm](api/vox-pm.md)
- [Crate API: vox-py](api/vox-py.md)
- [Crate API: vox-runtime](api/vox-runtime.md)
- [Crate API: vox-toestub](api/vox-toestub.md)
- [Crate API: vox-typeck](api/vox-typeck.md)
- [Crate API: vox-webhook](api/vox-webhook.md)
- [Decorators](api/decorators.md)
- [Deployment: Docker, Compose, Coolify, CI (SSOT)](reference/deployment-compose.md)
- [Doc inventory verifier (SSOT)](reference/doc-inventory.md)
- [Documentation Gaps](api/DOC_GAPS.md)
- [Environment variables (SSOT)](reference/env-vars.md)
- [Example: agent.vox](api/example_agent.md)
- [Example: Dashboard example: full-stack Vox with @v0 + routes + component + http route](api/example_dashboard.md)
- [Example: Data layer example — typed record store with Convex-inspired API.](api/example_data_layer.md)
- [Example: Durable Execution Example](api/example_durable_execution.md)
- [Example: Full-stack chatbot using server functions](api/example_chatbot_server_fn.md)
- [Example: greaterFool Reference Target in Vox](api/example_greaterfool_reference.md)
- [Example: Server function demo](api/example_server_fn.md)
- [Example: sharing.vox](api/example_sharing.md)
- [Example: Simple server function demo](api/example_simple_server_fn.md)
- [Example: TESTING](api/example_testing.md)
- [Example: Vox Actor Example](api/example_actor.md)
- [Example: Vox Chatbot Demo](api/example_chatbot.md)
- [Example: Vox Chatbot Pipeline (Compression Layer)](api/example_chatbot_pipeline.md)
- [Example: Vox Durable Actor Demo](api/example_durable_counter.md)
- [Example: Vox MCP Tool Example](api/example_mcp_tool.md)
- [Example: Vox Workflow Example](api/example_workflow.md)
- [External repositories & workspace SSOT](reference/external-repositories.md)
- [fn](api/keywords/fn.md)
- [for](api/keywords/for.md)
- [Hashing & Identity Builtins](reference/ref-builtins-hashing.md)
- [HF fine-tune gap matrix (SSOT ↔ code)](reference/hf-finetune-gap-matrix.md)
- [if](api/keywords/if.md)
- [import](api/keywords/import.md)
- [let](api/keywords/let.md)
- [match](api/keywords/match.md)
- [Mesh Coordination & Database Write Safety](reference/mesh-coordination.md)
- [Mesh SSOT (CPU-first)](reference/mesh.md)
- [Mobile and edge AI — SSOT](reference/mobile-edge-ai.md)
- [mut](api/keywords/mut.md)
- [Oratio & speech SSOT (Candle Whisper, no whisper.cpp)](reference/oratio-speech.md)
- [Populi Cloud GPU Training Strategy](reference/populi-cloud-gpu.md)
- [Populi LoRA / adapter ownership (vox-tensor vs vox-populi)](reference/populi-lora-ownership.md)
- [Populi native training SSOT (Burn LoRA, Candle qlora-rs QLoRA)](reference/populi-training.md)
- [QLoRA Fine-tuning Data Strategy & SSoT](reference/populi-qlora-data-strategy.md)
- [Reference: Decorator Registry](reference/ref-decorators.md)
- [Reference: Type System](reference/ref-type-system.md)
- [ret](api/keywords/ret.md)
- [Searching the Documentation](reference/ref-search.md)
- [Socrates protocol — single source of truth](reference/socrates-protocol.md)
- [spawn](api/keywords/spawn.md)
- [type](api/keywords/type.md)
- [Unified orchestration — SSOT](reference/orchestration-unified.md)
- [Vox Documentation Style Guide](reference/style-guide.md)
- [Vox full-stack build artifacts — single source of truth](reference/vox-fullstack-artifacts.md)
- [Vox full-stack web UI — single source of truth](reference/vox-web-stack.md)
- [Vox Language Guide](reference/ref-language.md)
- [Vox Skills Crate API](api/vox-skills.md)
- [vox-bootstrap](api/vox-bootstrap.md)
- [vox-dei (workspace-excluded)](api/vox-dei.md)
- [vox-ludus](api/vox-ludus.md)
- [vox-wasm](api/vox-wasm.md)
- [while](api/keywords/while.md)

# Archived / ADRs

- [ADR 001 — Burn Backend Selection for vox-tensor](adr/001-burn-backend-selection.md)
- [ADR 002 — Diátaxis Three-Tier Documentation Architecture](adr/002-diataxis-doc-architecture.md)
- [ADR 003 — Native Rust Training Over Python](adr/003-native-training-over-python.md)
- [ADR 004: Codex over Arca over Turso](adr/004-codex-arca-turso-ssot.md)
- [ADR 005: Socrates anti-hallucination SSOT](adr/005-socrates-anti-hallucination-ssot.md)
- [ADR 006: Populi full-graph Candle QLoRA with qlora-rs](adr/006-populi-full-graph-qlora-qlora-rs.md)
- [ADR 007: qlora-rs multi-layer training API (Phase 2c architecture gate)](adr/007-qlora-rs-multi-layer-training-api.md)
- [ADR 008: Mesh transport](adr/008-mesh-transport.md)
- [ADR 009: Hosted mesh / BaaS (future scope)](adr/009-mesh-hosted-baas.md)
- [ADR 010 — TanStack as the Vox web spine](adr/010-tanstack-web-spine.md)
- [Architecture Decision Records (ADR)](adr/README.md)
