# Summary

- [Cli Baseline Metrics](ci/cli-baseline-metrics.md)
- [Cli Command Surface.generated](reference/cli-command-surface.generated.md)
- [Cli Design Rules Ssot](architecture/cli-design-rules-ssot.md)
- [Cli Reachability Ssot](architecture/cli-reachability-ssot.md)
- [Command Compliance Ssot](ci/command-compliance-ssot.md)
- [Deployment Compose Ssot](architecture/deployment-compose-ssot.md)
- [Doc Inventory Ssot](ci/doc-inventory-ssot.md)
- [God Object Defactor Checklist](architecture/god-object-defactor-checklist.md)
- [Ludus Integration Contract](architecture/ludus-integration-contract.md)
- [Ludus Non Goals](architecture/ludus-non-goals.md)
- [Ludus Schema Inventory](architecture/ludus-schema-inventory.md)
- [Mcp Tool Registry Contract](reference/mcp-tool-registry-contract.md)
- [Mens Training Data Contract](reference/mens-training-data-contract.md)
- [Mens Training Ssot](architecture/mens-training-ssot.md)
- [Qwen35 Multimodal Phase2 Backlog](architecture/qwen35-multimodal-phase2-backlog.md)
- [Scaling Audit Baseline](architecture/scaling-audit-baseline.md)
- [Scaling Ci Enforcement](architecture/scaling-ci-enforcement.md)
- [Scaling Toestub Rules](architecture/scaling-toestub-rules.md)
- [Scholarly Digest Approval Invariants](architecture/scholarly-digest-approval-invariants.md)
- [Scholarly External Schema Plan](architecture/scholarly-external-schema-plan.md)
- [Script Surface Audit](architecture/script-surface-audit.md)
- [Ssot Convergence Roadmap](architecture/ssot-convergence-roadmap.md)
- [Table Spec Ssot](architecture/table-spec-ssot.md)
- [Telemetry Metric Contract](reference/telemetry-metric-contract.md)
- [Vox Web Stack Ssot](architecture/vox-web-stack-ssot.md)
- [Voxdb Connect Policy](architecture/voxdb-connect-policy.md)
- [Voxdb Cutover Runbook](operations/voxdb-cutover-runbook.md)
- [Vscode Mcp Compat](reference/vscode-mcp-compat.md)

# Getting Started

- [Vox: The AI-Native Programming Language](index.md)
- [Getting Started with Vox](tutorials/tut-getting-started.md)

# Tutorials

- [Tutorial: Actor Basics](tutorials/tut-actor-basics.md)
- [Tutorial: Building a Collaborative Todo List](tutorials/tut-first-app.md)
- [Tutorial: UI Integration](tutorials/tut-ui-integration.md)
- [Tutorial: Workflow Durability](tutorials/tut-workflow-durability.md)

# How-To Guides

- [@py.import – Python Library Integration (`torch`, `numpy`, etc.)](how-to/how-to-pytorch.md)
- [Contributing — Mens native training](how-to/how-to-contribute-mens.md)
- [Contributing — Populi control plane](how-to/how-to-contribute-populi.md)
- [Contributing — parser and HIR](how-to/how-to-contribute-parser-hir.md)
- [Ecosystem & Tooling](how-to/how-to-cli-ecosystem.md)
- [Glossary: Vox Terminology](explanation/glossary.md)
- [How To: Train Mens on RTX 4080 Super](how-to/how-to-train-mens-4080.md)
- [How to use the canonical VoxDB / Codex store](how-to/how-to-voxdb-canonical-store.md)
- [How-To: Build AI Agents and MCP Tools](how-to/how-to-ai-agents.md)
- [How-To: Deploy to Production](how-to/how-to-deploy.md)
- [How-To: Handle Errors Gracefully](how-to/how-to-error-handling.md)
- [How-To: Model Complex Domain Logic](how-to/how-to-custom-types.md)
- [How-To: Publish Scientia findings](how-to/how-to-scientia-publication.md)
- [How-To: Rust crate imports in Vox scripts](how-to/how-to-rust-crate-imports.md)
- [How-To: Test Your Logic](how-to/how-to-unit-test.md)
- [Model Routing & Provider Cascade](how-to/how-to-model-routing.md)
- [Scientia publication: what you type vs what the system derives](how-to/scientia-publication-operator-inputs.md)
- [Setup & Installation](how-to/how-to-setup.md)
- [Troubleshooting FAQ](how-to/troubleshooting-faq.md)
- [Vox FAQ: Frequently Asked Questions](explanation/faq.md)

# Examples

- [Golden Examples: Working Vox Code](examples/golden.md)

# Explanations

- [Actors & Workflows](explanation/expl-actors-workflows.md)
- [Compiler Architecture](explanation/expl-architecture.md)
- [Explanation: Compiler Lowering Phases](explanation/expl-compiler-lowering.md)
- [Explanation: Durable Execution](explanation/expl-durable-execution.md)
- [Explanation: Security Model](explanation/expl-security.md)
- [Explanation: The Vox Runtime](explanation/expl-runtime.md)
- [MCP Optimization Strategy](explanation/expl-mcp-optimizations.md)
- [Native ML Training Pipeline](explanation/expl-ml-pipeline.md)
- [OpenClaw Competitive Analysis](explanation/expl-openclaw-analysis.md)

# Architecture SSOTs

- [Anti-foot-gun planning standard](architecture/planning-meta/05-anti-foot-gun-planning-standard.md)
- [Document boundary matrix](architecture/planning-meta/11-document-boundary-matrix.md)
- [Document maintenance protocol](architecture/planning-meta/10-document-maintenance-protocol.md)
- [Exception and deferral policy](architecture/planning-meta/09-exception-deferral-policy.md)
- [Fast LLM instruction plan](architecture/planning-meta/02-fast-llm-instruction-plan.md)
- [Feature growth boundaries](architecture/feature-growth-boundaries.md)
- [Interop tier policy](architecture/interop-tier-policy.md)
- [Internal Web IR Implementation Blueprint](architecture/internal-web-ir-implementation-blueprint.md)
- [Internal Web IR Side-by-Side Schema](architecture/internal-web-ir-side-by-side-schema.md)
- [Master planning index](architecture/planning-meta/01-master-planning-index.md)
- [Mens lane segmentation research](architecture/mens-lane-segmentation-research.md)
- [Milestone and gate definition spec](architecture/planning-meta/08-milestone-gate-definition-spec.md)
- [Nomenclature migration map (SSOT)](architecture/nomenclature-migration-map.md)
- [Orchestrator AgentEventKind → Ludus matrix](architecture/agent-event-kind-ludus-matrix.md)
- [Planning critique and gap analysis](architecture/planning-meta/04-planning-critique-gap-analysis.md)
- [Planning meta exception register](architecture/planning-meta/exception-register.md)
- [Planning meta maintenance log](architecture/planning-meta/maintenance-log.md)
- [Planning taxonomy and glossary](architecture/planning-meta/06-planning-taxonomy-glossary.md)
- [Research baseline and source-of-truth map](architecture/planning-meta/00-research-baseline-source-map.md)
- [Task catalog authoring spec](architecture/planning-meta/07-task-catalog-authoring-spec.md)
- [Vox boilerplate implementation status](architecture/vox-boilerplate-implementation-status.md)
- [Vox boilerplate reduction master roadmap](architecture/vox-boilerplate-reduction-master-roadmap.md)
- [Vox bell-curve strategy](architecture/vox-bell-curve-strategy.md)
- [Vox boilerplate research findings 2026](architecture/vox-boilerplate-research-findings-2026.md)
- [Vox full-stack ergonomics deep dive](architecture/vox-fullstack-ergonomics-deep-dive.md)
- [Vox packaging full implementation plan 2026](architecture/vox-packaging-full-implementation-plan-2026.md)
- [Vox packaging implementation blueprint](architecture/vox-packaging-implementation-blueprint.md)
- [Vox packaging research findings 2026](architecture/vox-packaging-research-findings-2026.md)
- [Vox Docker-backed portability research 2026](architecture/vox-docker-dotvox-portability-research-2026.md)
- [VoxGiantia publication architecture (beginner map)](architecture/voxgiantia-publication-architecture.md)
- [Weighted deep planning manual](architecture/planning-meta/03-weighted-deep-planning-manual.md)

# Reference

- [@action](api/decorators/action.md)
- [@component](api/decorators/component.md)
- [@deprecated](api/decorators/deprecated.md)
- [@index](api/decorators/index.md)
- [@mcp.tool](api/decorators/mcp_tool.md)
- [@mutation](api/decorators/mutation.md)
- [@pure](api/decorators/pure.md)
- [@query](api/decorators/query.md)
- [@require](api/decorators/require.md)
- [@scheduled](api/decorators/scheduled.md)
- [@server](api/decorators/server.md)
- [@table](api/decorators/table.md)
- [@test](api/decorators/test.md)
- [@v0](api/decorators/v0.md)
- [ADR 001 — Burn Backend Selection for vox-tensor](adr/001-burn-backend-selection.md)
- [ADR 002 — Diátaxis Three-Tier Documentation Architecture](adr/002-diataxis-doc-architecture.md)
- [ADR 003 — Native Rust Training Over Python](adr/003-native-training-over-python.md)
- [ADR 004: Codex over Arca over Turso](adr/004-codex-arca-turso-ssot.md)
- [ADR 005: Socrates anti-hallucination SSOT](adr/005-socrates-anti-hallucination-ssot.md)
- [ADR 006: Mens full-graph Candle QLoRA with qlora-rs](adr/006-mens-full-graph-qlora-qlora-rs.md)
- [ADR 007: qlora-rs multi-layer training API (Phase 2c architecture gate)](adr/007-qlora-rs-multi-layer-training-api.md)
- [ADR 008: Mens transport](adr/008-populi-transport.md)
- [ADR 009: Hosted mens / BaaS (future scope)](adr/009-populi-hosted-baas.md)
- [ADR 010 — TanStack as the Vox web spine](adr/010-tanstack-web-spine.md)
- [ADR 011: Scientia publication manifest SSOT](adr/011-scientia-publication-ssot.md)
- [ADR 012 — Internal Web IR strategy for Vox](adr/012-internal-web-ir-strategy.md)
- [ADR 013 — OpenClaw WS-first native interop](adr/013-openclaw-ws-native-strategy.md)
- [Acceptance runbook — Mens HF fine-tune convergence](architecture/mens-finetune-acceptance-runbook.md)
- [Agent Messaging & Orchestration Roadmap (Aspirational)](architecture/agent-messaging-orchestration-roadmap.md)
- [Architecture Decision Records (ADR)](adr/README.md)
- [Automation primitives](architecture/vox-automation-primitives.md)
- [Binary release artifact contract](ci/binary-release-contract.md)
- [Boilerplate metrics and KPI framework](ci/boilerplate-metrics.md)
- [CI runner contract](ci/runner-contract.md)
- [CLI reference (redirect)](ref-cli.md)
- [CLI scope policy](architecture/cli-scope-policy.md)
- [Changelog](reference/changelog.md)
- [Clavis SSOT](reference/clavis-ssot.md)
- [Codex / Arca compatibility boundaries](architecture/codex-arca-compatibility-boundaries.md)
- [Codex BaaS scaffolding](architecture/codex-baas.md)
- [Codex HTTP API](reference/codex-http-api.md)
- [Codex legacy migration](architecture/codex-legacy-migration.md)
- [Codex vNext — schema domains](architecture/codex-vnext-schema.md)
- [Codex, Arca, and Rust import policy](architecture/codex-arca-import-policy.md)
- [Command compliance](reference/command-compliance.md)
- [Command surface duals (intentional)](ci/command-surface-duals.md)
- [Compatibility and deprecation windows](architecture/compatibility-deprecation-windows.md)
- [Crate API: vox-ast](api/vox-ast.md)
- [Crate API: vox-codegen-rust](api/vox-codegen-rust.md)
- [Crate API: vox-codegen-ts](api/vox-codegen-ts.md)
- [Crate API: vox-config](api/vox-config.md)
- [Crate API: vox-db](api/vox-db.md)
- [Crate API: vox-doc-pipeline](api/vox-doc-pipeline.md)
- [Crate API: vox-gamify](api/vox-gamify.md)
- [Crate API: vox-hir](api/vox-hir.md)
- [Crate API: vox-integration-tests](api/vox-integration-tests.md)
- [Crate API: vox-lexer](api/vox-lexer.md)
- [Crate API: vox-lsp](api/vox-lsp.md)
- [Crate API: vox-mcp](api/vox-mcp.md)
- [Crate API: vox-parser](api/vox-parser.md)
- [Crate API: vox-pm](api/vox-pm.md)
- [Crate API: vox-py](api/vox-py.md)
- [Crate API: vox-runtime](api/vox-runtime.md)
- [Crate API: vox-toestub](api/vox-toestub.md)
- [Crate API: vox-typeck](api/vox-typeck.md)
- [Crate API: vox-webhook](api/vox-webhook.md)
- [Crate and build-lane migration map](architecture/crate-build-lanes-migration.md)
- [Crate hardening matrix (rolling)](ci/crate-hardening-matrix.md)
- [Crate topology buckets](architecture/crate-topology-buckets.md)
- [Cross-platform Vox — runbook](architecture/vox-cross-platform-runbook.md)
- [Decorators](api/decorators.md)
- [Deployment: Docker, Compose, Coolify, CI (SSOT)](reference/deployment-compose.md)
- [Deprecation policy — Mens native fine-tuning](architecture/mens-finetune-deprecation.md)
- [Diagnostic taxonomy (compiler)](reference/diagnostic-taxonomy.md)
- [Direct `turso::` usage allowlist](architecture/codex-turso-allowlist.md)
- [Doc inventory verifier (SSOT)](reference/doc-inventory.md)
- [Doc-to-code acceptance checklist](architecture/doc-to-code-acceptance-checklist.md)
- [Docker image baselines (D05)](reference/docker-image-baselines.md)
- [Documentation Gaps](api/DOC_GAPS.md)
- [Environment variables (SSOT)](reference/env-vars.md)
- [Environment variables (SSOT) (redirect)](ref/env-vars.md)
- [Environment variables SSOT filename (redirect)](ref/env-vars-ssot.md)
- [Standard library surfaces](reference/std-surfaces.md)
- [Example: Dashboard example: full-stack Vox with @v0 + routes + component + http route](api/example_dashboard.md)
- [Example: Data layer example — typed record store with Convex-inspired API.](api/example_data_layer.md)
- [Example: Durable Execution Example](api/example_durable_execution.md)
- [Example: Full-stack chatbot using server functions](api/example_chatbot_server_fn.md)
- [Example: Server function demo](api/example_server_fn.md)
- [Example: Simple server function demo](api/example_simple_server_fn.md)
- [Example: TESTING](api/example_testing.md)
- [Example: Vox Actor Example](api/example_actor.md)
- [Example: Vox Chatbot Demo](api/example_chatbot.md)
- [Example: Vox Chatbot Pipeline (Compression Layer)](api/example_chatbot_pipeline.md)
- [Example: Vox Durable Actor Demo](api/example_durable_counter.md)
- [Example: Vox MCP Tool Example](api/example_mcp_tool.md)
- [Example: Vox Workflow Example](api/example_workflow.md)
- [Example: agent.vox](api/example_agent.md)
- [Example: greaterFool Reference Target in Vox](api/example_greaterfool_reference.md)
- [Example: sharing.vox](api/example_sharing.md)
- [Examples](how-to/examples.md)
- [Examples corpus & strict parse](how-to/examples-corpus.md)
- [Explicitly out of scope for Rust migration](architecture/external-scripts-boundary-archive.md)
- [External repositories & workspace SSOT](reference/external-repositories.md)
- [Feasibility: full-graph Candle training (qlora-rs)](architecture/candle-full-graph-feasibility.md)
- [First full-stack Vox app](how-to/first-full-stack-app.md)
- [Forward-only migration charter](architecture/forward-migration-charter.md)
- [GitHub-hosted runner exceptions](ci/github-hosted-exceptions.md)
- [HF fine-tune gap matrix (SSOT ↔ code)](reference/hf-finetune-gap-matrix.md)
- [HF fine-tuning capability matrix (code-grounded)](architecture/hf-finetune-capability-matrix.md)
- [HIR legacy AST wrappers (inventory)](reference/hir-legacy-inventory.md)
- [Hashing & Identity Builtins](reference/ref-builtins-hashing.md)
- [Information-theoretic questioning protocol](reference/information-theoretic-questioning.md)
- [Language ergonomics principles](reference/language-ergonomics-principles.md)
- [MENS curriculum — speech-to-code stages](reference/mens-speech-curriculum.md)
- [MENS findings: Composer and Kimi (2026)](architecture/mens-composer-kimi-findings-2026.md)
- [Mens / HF fine-tune — LLM PR checklist](architecture/mens-llm-pr-checklist.md)
- [Mens Architecture 2026 Synthesis](architecture/mens-architecture-2026-synthesis.md)
- [Mens Cloud GPU Training Strategy](reference/mens-cloud-gpu.md)
- [Mens Coordination & Database Write Safety](reference/populi-coordination.md)
- [Mens Coordination Workflow Guide](architecture/populi-workflow-guide.md)
- [Mens LoRA / adapter ownership (vox-tensor vs vox-populi)](reference/mens-lora-ownership.md)
- [Mens external technology options](reference/mens-external-tech-options.md)
- [Mens laziness and accuracy audit](reference/mens-laziness-accuracy-audit.md)
- [Mens measurement gap analysis](reference/mens-measurement-gap-analysis.md)
- [Mens native training SSOT (Candle QLoRA–first; Burn LoRA deprecated in dispatch)](reference/mens-training.md)
- [Mens strategy inputs checklist](reference/mens-strategy-inputs-checklist.md)
- [Mesh / Populi SSOT (CPU-first)](reference/populi.md)
- [Migration metrics (script → `vox ci`)](architecture/migration-script-dashboard.md)
- [Migration: backend-centric flags → fine-tune contract](architecture/mens-finetune-migration.md)
- [Mobile and edge AI — SSOT](reference/mobile-edge-ai.md)
- [Oratio & speech SSOT (Candle Whisper, no whisper.cpp)](reference/oratio-speech.md)
- [Orphan surface inventory](architecture/orphan-surface-inventory.md)
- [Package management migration (2026)](reference/pm-migration-2026.md)
- [Parser ambiguity and robustness inventory](reference/parser-ambiguity-inventory.md)
- [Parser feature matrix](reference/parser-feature-matrix.md)
- [Phase 0 documentation baseline — signoff](architecture/phase0-migration-signoff.md)
- [QLoRA Fine-tuning Data Strategy & SSoT](reference/mens-qlora-data-strategy.md)
- [Reference: Decorator Registry](reference/ref-decorators.md)
- [Reference: Type System](reference/ref-type-system.md)
- [Research Notes: Achieving Serverless-like Performance with MCP](explanation/mcp_serverless_research.md)
- [Rust pattern modernization — Wave 0 baseline](ci/rust-modernization-baseline.md)
- [SCIENTIA SSOT handbook (glossary, vocabulary, checklists)](reference/scientia-ssot-handbook.md)
- [SCIENTIA publication automation SSOT](architecture/scientia-publication-automation-ssot.md)
- [SCIENTIA publication readiness audit](architecture/scientia-publication-readiness-audit.md)
- [SCIENTIA publication worthiness rules](reference/scientia-publication-worthiness-rules.md)
- [Scientia publication failure playbook](reference/scientia-publication-playbook.md)
- [Searching the Documentation](reference/ref-search.md)
- [Socrates protocol — single source of truth](reference/socrates-protocol.md)
- [Trust Reliability Layer (SSOT)](architecture/trust-reliability-layer.md)
- [Speech-to-code pipeline (Oratio → MCP → compiler → MENS)](reference/speech-to-code-pipeline.md)
- [Speech-to-code — operations, security, rollout](reference/speech-to-code-operations.md)
- [Syntax K complexity telemetry (WebIR + emit)](reference/syntax-k-complexity.md)
- [TOESTUB self-healing architecture 2026](architecture/toestub-self-healing-architecture-2026.md)
- [TanStack SSR with Axum (development topology)](how-to/tanstack-ssr-with-axum.md)
- [TanStack web backlog](architecture/tanstack-web-backlog.md)
- [TanStack web roadmap](architecture/tanstack-web-roadmap.md)
- [Testing Standard — SSOT](architecture/testing-standard.md)
- [Trim, build, and defer (feature lifecycle)](architecture/trim-build-defer-policy.md)
- [TypeScript boundary policy](architecture/typescript-migration-boundary.md)
- [Unified orchestration — SSOT](reference/orchestration-unified.md)
- [Vox DEI (Distributed Execution Intelligence)](api/vox-dei.md)
- [Vox Documentation Style Guide](reference/style-guide.md)
- [Vox Language Guide](reference/ref-language.md)
- [Vox Skills Crate API](api/vox-skills.md)
- [Vox Web Architecture Analysis](architecture/web-architecture-analysis-2026.md)
- [Vox database language surface (canonical)](reference/vox-db-language-surface.md)
- [Vox full-stack build artifacts — single source of truth](reference/vox-fullstack-artifacts.md)
- [Vox full-stack web UI — single source of truth](reference/vox-web-stack.md)
- [Workflow enumeration (GitHub Actions)](ci/workflow-enumeration.md)
- [Workspace root `Cargo.toml` (fix forward)](ci/workspace-root-manifest.md)
- [Zig-Inspired Deployment Architecture](explanation/zig-inspired-deployment.md)
- [actor](api/keywords/actor.md)
- [await](api/keywords/await.md)
- [cli](reference/cli.md)
- [fn](api/keywords/fn.md)
- [for](api/keywords/for.md)
- [if](api/keywords/if.md)
- [import](api/keywords/import.md)
- [let](api/keywords/let.md)
- [match](api/keywords/match.md)
- [mut](api/keywords/mut.md)
- [ret](api/keywords/ret.md)
- [spawn](api/keywords/spawn.md)
- [type](api/keywords/type.md)
- [vox-bootstrap](api/vox-bootstrap.md)
- [vox-cli build and feature inventory](architecture/vox-cli-build-feature-inventory.md)
- [vox-dei (workspace-excluded)](api/vox-dei-sandbox.md)
- [vox-ludus](api/vox-ludus.md)
- [vox-wasm](api/vox-wasm.md)
- [while](api/keywords/while.md)

# tutorials

- [Tutorial: first .vox app (checkpoints)](tutorials/tut-first-vox-app-checkpoints.md)

