...
- Optional **`--install`**: installs `vox` after probe success.
  ...
  - **Source fallback:** `cargo install --path crates/vox-cli` from repo root discovered via **`VOX_REPO_ROOT`** or by walking up from the current directory until `crates/vox-cli/Cargo.toml` exists. If only standalone `vox-bootstrap` is present (no local repo + Cargo), source fallback cannot run.
