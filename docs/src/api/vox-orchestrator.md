---
title: "Vox DEI (Distributed Execution Intelligence)"
description: "Official documentation for Distributed Execution Intelligence (DEI) for the Vox language. Detailed technical reference, architecture guides, and implementation."
category: "reference"
...
**Process model:** `Orchestrator` is a library type in `vox-orchestrator`; the usual MCP-facing process is **`vox-mcp`** (stdio server) which embeds orchestrator state. See [`agents/orchestrator.md`](../../agents/orchestrator.md) and [`crates/vox-mcp/src/tools/mod.rs`](../../../crates/vox-mcp/src/tools/mod.rs) (`TOOL_REGISTRY`).

**Bootstrap:** first-party surfaces use **`vox_orchestrator::build_repo_scoped_orchestrator`** ([`bootstrap.rs`](../../../crates/vox-orchestrator/src/bootstrap.rs)) so `repository_id`, affinity groups, and memory shard paths match between MCP, **`vox dei`**, and other CLI entrypoints. See [ADR 022](../adr/022-orchestrator-bootstrap-and-daemon-boundaries.md).

**Mesh:** with **`VOX_MESH_ENABLED`**, durable locks / oplog / A2A / heartbeats sync via Turso while each node keeps its own in-memory `Orchestrator`; see [Mens coordination](../reference/populi-coordination.md) and [Unified orchestration](../reference/orchestration-unified.md).
...
```bash
vox dei submit "fix bug" --files src/main.vox
vox dei status
```
