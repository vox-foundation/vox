...
> **amended (2026-05-08):** Implemented for the Rust target + typeck only. **TS codegen lowering is deferred** (A3) — the existing TS codegen has no special handling for any `std.*` namespace today (`std.json`, `std.fs`, etc. all emit raw `std.namespace.method(...)` calls that don't run in the browser); plumbing TS-side regex would require a full std.namespace dispatch layer that's out of scope for this PR. **A6 (`docs/src/reference/stdlib-regex.md`) deferred** — `docs/src/reference/` doesn't have stdlib-* pages yet. The `regex` crate is added to `vox-runtime`'s Cargo.toml (workspace-deps already had it). Runtime helpers `vox_regex_compile`, `VoxRegex::{matches,find,find_all}`, `VoxMatch::group` live in `crates/vox-runtime/src/builtins/mod.rs`. Compiler wiring: `std.regex` namespace + `compile` method registered in `builtin_registry.rs`; `StdRegexNs` arm in `typeck/checker/expr_field.rs` and `expr.rs`; `Regex` and `Match` types' methods in `typeck/builtins.rs`.
