...
> **Correction (post-Task-3, 2026-09-11):** this plan's premise below — "the memory model is fitted from MLX points only" / "there is no measured CUDA `a_lane`" (see the Entry Gate, the Task 3 step 5 excerpt, and the Known Gaps section) — was found to be backwards during Task 3's execution and is now confirmed false against the landed code. There is no MLX `Lane` variant at all: the real lanes are `CandleCuda` and `CandleMetal` (`crates/vox-populi/src/mens/tensor/memory_model.rs`). `CandleCuda` carries a **seeded** row (inherited from the pre-SSOT `memory_budget.rs` constants, `CalSource::Seeded`) and is what `vox mens cloud-estimate` sizes every rented (CUDA) offer against; `CandleMetal` is the lane that is genuinely uncalibrated today (one real measurement, too few to fit a slope) and fails closed. This plan's text below is left as-authored — a dated intent snapshot, not rewritten — see the module doc comment at the top of `memory_model.rs` and `docs/superpowers/plans/2026-09-11-1-memory-ssot-and-fit-benchmark.md` (the memory-SSOT plan) for the fuller, current explanation.
