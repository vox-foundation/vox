...
**Update 2026-09-25:** file-backed databases did not reproduce the corruption (0/300 trials); it is a Turso `:memory:` engine bug (see the 2026-09-25 section of `docs/src/architecture/2026-09-22-voxdb-turso-pooling-and-mvcc-recommendation.md`). The two `pool.rs` tests were replaced by one default-run yielding test on a temp file, and the `:memory:` reproducers moved to `crates/vox-db/tests/pool_corruption_probe.rs`.
