...
`agent_loop` after a tool result: if `needs_human`, **stop iterating** (do not call the model again with “try another click”). Emit a chat event that the turn is waiting on the operator. Block on the daemon-local oneshot / notify for that `page_id` (or conversation id if no page). Resume only when `resume_axis` fires.
...
- `vox_browser_screenshot_viewport` and `vox_browser_screencast_frame` return the same small JSON (`page_id`, `path` if already on disk, `width`, `height`, `mime`) **without** an MCP image part and **without** writing a new frame under `browser-frames/` for this call.
...
2. The verb **does not execute**. Record `PendingSensitive` in `browser_hitl.rs`.
...
4. Operator either:
   ...
   - Hits **Confirm** once: replay that tool with `CallerContext::gui()` (Human). Lock stays whatever the toolbar currently is (Confirm does not force You).
...
| Mute | Agent + `lock=human` → `mcp_contents_for_tool_json` has no image part; no new cache write |
...
| Confirm forces You mode | Rejected — toolbar stays as the operator left it |
