...
| `populi_release_gate.sh` / `.ps1` (legacy wrappers) | **`vox ci populi-gate --profile m1m4`** |
| `populi/release_training_gate.sh` / `.ps1` | `vox ci populi-gate --profile training` |
| `run_qwen25_qlora_real_4080.ps1` | Optional **operator** helper: CUDA (or CPU Candle) build + background **`vox populi train --backend qlora …`**; canonical command is still **`vox populi train`** (see `docs/src/architecture/populi-training-ssot.md`). |

Populi gate steps live in **`scripts/populi/gates.yaml`**. **`ci_full`** (run in GitHub `ci.yml` after `cargo test --workspace`) is the broad CI profile: **`vox ci populi-gate --profile ci_full`**. **`m1m4`** / **`training`** are narrower release/training profiles used by the shell wrappers above.
