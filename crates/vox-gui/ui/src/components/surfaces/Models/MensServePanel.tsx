...
export function MensServePanel({ pushToast }: MensServePanelProps) {
  ...
  useEffect(() => {
    ...
    invoke<MensServeStatus>('mens_serve_status')
      ...
      .catch((err) => {
        if (!cancelled) {
          pushToast({ tone: 'warn', title: 'mens serve status failed', body: sanitizeErrorForToast(err), cause: 'backend-error' });
        }
      });
    return () => {
      cancelled = true;
  ...
  const start = async () => {
    ...
    try {
      ...
      pushToast({ tone: 'ok', title: 'mens serve started', body: model, cause: 'backend-ok' });
    } catch (err) {
      pushToast({ tone: 'warn', title: 'mens serve start failed', body: sanitizeErrorForToast(err), cause: 'backend-error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const stop = async () => {
    ...
    try {
      ...
      pushToast({ tone: 'warn', title: 'mens serve stop failed', body: sanitizeErrorForToast(err), cause: 'backend-error' });
    } finally {
      setBusy(false);
    }
  };
