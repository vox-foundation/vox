// Top HUD bar — KPIs + sparklines
const { useState: useState_hud } = React;

function KPI({ label, value, unit, delta, color, spark, icon, sub }) {
...
function TopHud({ kpis, onCommand }) {
  return (
    <Glass className="flex items-stretch overflow-hidden">
      ...
      <div className="flex items-stretch divide-x divide-white/5">
        <KPI label="Active Agents" value={kpis.activeAgents.value} delta={kpis.activeAgents.delta} color="text-cyan-300" spark={kpis.activeAgents.spark} icon={<Icon.cpu className="size-4"/>} />
        <KPI label="Queue Depth"   value={kpis.queueDepth.value}   delta={kpis.queueDepth.delta}   color="text-zinc-300" spark={kpis.queueDepth.spark} icon={<Icon.scale className="size-4"/>} />
        <KPI label="Budget Burn"   value={`$${kpis.budgetBurn.value.toFixed(2)}`} unit={`/ $${kpis.budgetBurn.cap.toFixed(2)}`} delta={kpis.budgetBurn.delta} color="text-amber-300" spark={kpis.budgetBurn.spark} icon={<Icon.flame className="size-4"/>} sub={`${Math.round(kpis.budgetBurn.value/kpis.budgetBurn.cap*100)}% of cap`} />
        <KPI label="Mesh Throughput" value={kpis.mesh.value} unit={kpis.mesh.unit} delta={kpis.mesh.delta} color="text-violet-300" spark={kpis.mesh.spark} icon={<Icon.cpu className="size-4"/>}
             sub={`${kpis.mesh.peers} peers online · ${kpis.mesh.vramGb}GB VRAM`} />
      </div>
      ...
      <div className="ml-auto flex items-center gap-2 px-4">
        <button onClick={onCommand} className="group flex items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-1.5 text-xs text-zinc-400 hover:border-brass/40 hover:text-brass transition">
          ...
          <span className="font-display tracking-wider">Command</span>
