...
pub async fn get_profile(db: &Codex, user_id: &str) -> Result<Option<LudusProfile>> {
    if let Some(row) = db.store().get_gamify_profile_raw(user_id).await? {
...
pub async fn unlock_achievement(
    ...
    Ok(db.store().unlock_gamify_achievement(
        user_id,
        achievement_id,
        now,
        xp as i64,
...
pub async fn record_level_up(
    ...
    let now = crate::util::now_unix();
    db.store().record_gamify_level_up(
        user_id,
        level as i64,
        title,
        xp_at_level as i64,
...
pub async fn list_unlocked_achievements(db: &Codex, user_id: &str) -> Result<Vec<(String, i64)>> {
    Ok(db.store().list_gamify_achievements(user_id).await?)
}
...
pub async fn list_companions(db: &Codex, user_id: &str) -> Result<Vec<Companion>> {
    let rows = db.store().list_gamify_companions(user_id).await?;
...
pub async fn upsert_companion(db: &Codex, c: &Companion) -> Result<()> {
    db.store().upsert_gamify_companion(
        &c.id,
        &c.user_id,
        &c.name,
        c.description.clone(),
...
pub async fn get_companion(db: &Codex, id: &str) -> Result<Option<Companion>> {
    let row = db.store().get_gamify_companion(id).await?;
    if let Some(row) = row {
...
pub async fn delete_companion(db: &Codex, id: &str) -> Result<()> {
    db.store().delete_gamify_companion(id).await?;
    Ok(())
}
...
pub async fn list_quests(db: &Codex, user_id: &str) -> Result<Vec<Quest>> {
    let rows = db.store().list_gamify_quests(user_id).await?;
...
pub async fn update_quest_status(db: &Codex, user_id: &str, id: &str, status: &str) -> Result<()> {
    ...
    db.store().update_gamify_quest_status(id, user_id, status, completed).await?;
    Ok(())
}

/// Count active/available quests for a user.
pub async fn count_quests(db: &Codex, user_id: &str) -> Result<u32> {
    Ok(db.store().count_gamify_quests(user_id).await? as u32)
}

/// Delete a quest.
pub async fn delete_quest(db: &Codex, id: &str) -> Result<()> {
    db.store().delete_gamify_quest(id).await?;
    Ok(())
}
...
pub async fn list_battles(db: &Codex, user_id: &str, limit: i64) -> Result<Vec<Battle>> {
    let rows = db.store().list_gamify_battles(user_id, limit).await?;
...
pub async fn count_battles(db: &Codex, user_id: &str) -> Result<i64> {
    Ok(db.store().count_gamify_battles(user_id).await?)
}
...
pub async fn leaderboard(db: &Codex, limit: i64) -> Result<Vec<PlayerRankEntry>> {
    let rows = db.store().gamify_leaderboard_by_xp(limit).await?;
...
pub async fn lumens_leaderboard(db: &Codex, limit: i64) -> Result<Vec<PlayerRankEntry>> {
    let rows = db.store().gamify_leaderboard_by_lumens(limit).await?;
    let mut entries = Vec::new();
    for (user_id, level, score) in rows {
        entries.push(PlayerRankEntry {
            user_id,
...
pub async fn get_profile_stats(db: &Codex, user_id: &str) -> Result<serde_json::Value> {
    let (completed_quests, won_battles) = db.store().get_gamify_profile_stats(user_id).await?;
    Ok(serde_json::json!({
...
pub async fn get_events(
    ...
    let rows = db.store().list_gamify_events(agent_id, limit.unwrap_or(50)).await?;
...
pub async fn insert_event(
    ...
    db.store().insert_gamify_event(agent_id, event_type, payload).await?;
    Ok(())
}
...
pub async fn insert_cost_record(
    ...
    db.store().insert_gamify_cost_record(agent_id, session_id, provider, model, input_tokens, output_tokens, cost_usd).await?;
    Ok(())
}

/// Get total cost for an agent.
pub async fn get_agent_cost_usd(db: &Codex, agent_id: &str) -> Result<f64> {
    Ok(db.store().get_gamify_agent_cost_usd(agent_id).await?)
}
...
pub async fn list_cost_records(db: &Codex, agent_id: &str, limit: i64) -> Result<Vec<CostRecord>> {
    let rows = db.store().list_gamify_cost_records(agent_id, limit).await?;
...
pub async fn acknowledge_message(db: &Codex, id: i64) -> Result<()> {
    db.store().acknowledge_a2a_message_by_id(id).await?;
    Ok(())
}
...
pub async fn insert_agent_session(
    ...
    db.store().insert_gamify_session(id, agent_id, agent_name).await?;
    Ok(())
}
...
pub async fn upsert_agent_metric(
    ...
    db.store().upsert_gamify_agent_metric(agent_id, metric_name, metric_value, period).await?;
    Ok(())
}
...
pub async fn get_agent_metrics(
    ...
    let metrics = db.store().get_gamify_agent_metrics(agent_id, period).await?;
