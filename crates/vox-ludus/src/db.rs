...
/// Persistent agent-to-agent message record for audit and inbox delivery.
#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct A2AMessageRecord {
    ...
    pub sender: String,
    /// Agent ID of the intended recipient, or `None` for broadcast messages.
    pub receiver: Option<String>,
    /// Message type discriminant (e.g. `"question"`, `"answer"`, `"broadcast"`).
    pub msg_type: String,
    /// Optional JSON payload body.
    pub payload: Option<String>,
    /// Optional correlation ID for request-reply pairing.
    pub correlation_id: Option<String>,
    /// Whether the recipient has acknowledged receipt.
    pub acknowledged: bool,
    /// SQLite `datetime` string when the message was stored.
    pub timestamp: String,
}
...
pub async fn get_pending_messages(db: &Codex, receiver: &str) -> Result<Vec<A2AMessageRecord>> {
    let mut rows = db.store().conn.query(
...
pub async fn list_a2a_messages(db: &Codex, limit: i64) -> Result<Vec<A2AMessageRecord>> {
    let mut rows = db.store().conn.query(
        "SELECT id, sender, receiver, msg_type, payload, correlation_id, acknowledged, timestamp
