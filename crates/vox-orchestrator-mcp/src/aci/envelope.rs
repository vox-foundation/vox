...
fn aci_shell_backend_for_tool(canon: &str) -> Value {
    if canon == "vox_run_shell" {
        Value::String("powershell".into())
    } else {
        Value::Null
    }
}
...
pub fn attach_aci_envelope(
    ...
    aci_obj.insert("shell_backend".into(), aci_shell_backend_for_tool(canon));
...
mod tests {
    ...
    fn aci_side_effects_for_local_mutation_tool() {
        ...
        let out = attach_aci_envelope("vox_write_file", inner, false).expect("attach");
        let v: Value = serde_json::from_str(&out).expect("parse");
    ...
    #[test]
    fn aci_shell_backend_for_run_shell() {
        let inner = r#"{"success":true,"data":"ok"}"#;
        let out = attach_aci_envelope("vox_run_shell", inner, false).expect("attach");
        let v: Value = serde_json::from_str(&out).expect("parse");
