...
async fn test_orchestrator_a2a_message_bus_integration() {
    ...
    let msg_id = orch.message_bus.send(agent1, agent2, A2AMessageType::HelpRequest, "Need assistance with testing");
    
    // Verify the message reached agent2's inbox
    let inbox2 = orch.message_bus.inbox(agent2);
    assert_eq!(inbox2.len(), 1, "Agent B should have received the A2A message");
    let msg = &inbox2[0];
    ...
    assert!(trail.iter().any(|m| m.id == msg_id), "Message must be in the audit trail");
}
