...
use vox_orchestrator::types::{A2AMessageType, AgentId, MessagePriority};
...
async fn test_distributed_coordination_primitives() {
    ...
    // 2. Heartbeats
    heartbeat::persist_heartbeat_with_breaker(
        &db,
        "node-1",
        AgentId(1),
        "writing".parse().unwrap(),
        repo_id,
    )
    .await
    ...
    // 4. Distributed Locks
    let lock_res = locks::acquire_distributed_lock_with_breaker(
        &db,
        "file.rs",
        "node-1",
        AgentId(1),
    ...
    let lock_res_2 = locks::acquire_distributed_lock_with_breaker(
        &db,
        "file.rs",
        "node-2",
        AgentId(2),
    ...
    let lock_res_3 = locks::acquire_distributed_lock_with_breaker(
        &db,
        "file.rs",
        "node-2",
        AgentId(2),
    ...
    // 5. OpLog
    let op_kind = oplog::OperationKind::Custom {
        label: "test".into(),
    };
