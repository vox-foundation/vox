    ...
    pub async fn complete_task_with_attestation(
        ...
        if let Some(db) = self.db() {
            if task_clone_opt.is_some() {
                let rows = db
                    .list_trust_scores_for_dimension("agent", "task_completion", Some(phase_label.as_str()), 512)
                    .await
                    .unwrap_or_default();
        ...
        let broken_links_report = if task_clone_opt.is_some() {
            crate::orchestrator::task_dispatch::complete::audit_reporter::verify_broken_links(&write_files).await
        } else {
            String::new()
        };

        if let Some(ref t) = task_clone_opt {
            self.apply_vox_healing(agent_id, &t.description, &write_files).await?;
        }

        let mut behavioral_failure = None;
        if task_clone_opt.is_some() {
            let gate = crate::gate::BehavioralGate::new(true);
            if let crate::gate::GateResult::BehavioralTestFailed { message } = gate.check_behavior(None).await {
...

        {
            let queue_lock = {
                let agents = crate::sync_lock::rw_read(&*self.agents);
                agents.get(&agent_id).ok_or(OrchestratorError::AgentNotFound(agent_id))?.clone()
            };
            let mut queue = crate::sync_lock::rw_write(&*queue_lock);
            ...
            if let Some(ref task) = queue.current_task() {
                ...
                if auto_debug_requeue.is_none() {
                    match gates::check_research_json_gate(task, completion_attestation.as_ref(), max_debug_iterations) {
                ...
                // Approval gate
                if auto_debug_requeue.is_none() {
                    match gates::check_approval_gate(task, completion_attestation.as_ref(), max_debug_iterations) {
                        Ok(outcome) => auto_debug_requeue = outcome.requeue,
                ...
                // Trust gate
                if auto_debug_requeue.is_none() {
                    let outcome = gates::check_trust_gate(task, completion_attestation.as_ref(), trust_score_opt.unwrap_or(0.5), trust_floor, max_toestub_debug_iterations, &phase_label);
                    auto_debug_requeue = outcome.requeue;
                }

                // Harness gate
                if auto_debug_requeue.is_none() {
                    match gates::check_harness_gate(task, completion_attestation.as_ref(), max_debug_iterations) {
                        Ok(outcome) => auto_debug_requeue = outcome.requeue,
                ...
                if auto_debug_requeue.is_none() && crate::sync_lock::rw_read(&*self.config).toestub_gate {
                    match gates::check_toestub_gate(task, completion_attestation.as_ref(), max_toestub_debug_iterations) {
                        Ok(outcome) => auto_debug_requeue = outcome.requeue,
                        Err(e) => {
                            if e.contains("Lock conflict") {
                                // Extract the message from "Lock conflict: <msg>"
                                let _msg = e.replace("Lock conflict: ", "");
                                return Err(OrchestratorError::LockConflict(crate::locks::LockConflict::ExclusivelyHeld { 
                ...
                // Doc integrity gate
                if auto_debug_requeue.is_none() {
                    let outcome = gates::check_doc_integrity_gate(task, &broken_links_report, &write_files, max_debug_iterations);
                    auto_debug_requeue = outcome.requeue;
                }
            }

            if let Some((requeue_task, err_report, _err_n, _warn_n)) = auto_debug_requeue {
                tracing::warn!("Task {} failed validation. Auto-debugging (iteration {})", task_id, requeue_task.debug_iterations);
                queue.mark_failed(task_id, format!("Auto-debug validation failure:\n{}", err_report));
                self.record_task_loop_metric(task_id, &phase_label, "toestub_requeue", requeue_task.debug_iterations);
        ...
        let socrates_outcome = {
            let (task_clone, max_socrates_iterations) = {
                let agents = crate::sync_lock::rw_read(&*self.agents);
                let queue_lock = agents.get(&agent_id).ok_or(OrchestratorError::AgentNotFound(agent_id))?;
                let queue = crate::sync_lock::rw_read(&**queue_lock);
                let t = queue.current_task().cloned().ok_or(OrchestratorError::TaskNotFound(task_id))?;
                let cfg = crate::sync_lock::rw_read(&*self.config);
                (t, cfg.max_socrates_debug_iterations)
            };
            
            let trust_relax_min = crate::sync_lock::rw_read(&*self.config).trust_gate_relax_min_reliability;
            ...
            self.check_socrates_gate(task_id, agent_id, &task_clone, completion_attestation.as_ref(), max_socrates_iterations, trust_relax_gates).await?
        };
        ...
        if let Some((requeue_task, err_report, _err_n, _warn_n)) = socrates_outcome.requeue {
            ...
            let queue_lock = agents.get(&agent_id).ok_or(OrchestratorError::AgentNotFound(agent_id))?;
            let mut queue = crate::sync_lock::rw_write(&**queue_lock);
            tracing::warn!("Task {} failed Socrates validation. Auto-debugging (iteration {})", task_id, requeue_task.debug_iterations);
            queue.mark_failed(task_id, format!("Auto-debug Socrates validation failure:\n{}", err_report));
            self.record_task_loop_metric(task_id, &phase_label, "toestub_requeue", requeue_task.debug_iterations as u8);
            queue.enqueue(requeue_task);
            return Ok(());
        }
        ...
        let completion_data = {
            ...
            let current = queue.current_task().cloned().ok_or(OrchestratorError::TaskNotFound(task_id))?;
            let plan_meta = match (current.plan_session_id.clone(), current.plan_node_id.clone(), current.plan_version) {
        ...
        let (write_files, session_id, desc, phase_label, debug_iterations, plan_meta, campaign_id, benchmark_tier, audit_report) = completion_data;

        let (snap_before, db_snap_before) = crate::sync_lock::rw_read(&*self.oplog).find_task_snapshots(task_id.0);
        let snapshot_after = self.capture_snapshot(agent_id, &write_files, format!("post-task complete: {:.50}", desc)).await;
        let db_snap_after = self.take_db_snapshot(agent_id, format!("post-task-complete: {}", task_id)).await;

        self.record_operation(agent_id, crate::oplog::OperationKind::TaskComplete { task_id: task_id.0 }, format!("Completed task {}", task_id), snap_before, Some(snapshot_after), db_snap_before, db_snap_after).await;

        self.record_success_persistence(task_id, agent_id, session_id.clone(), &phase_label, &desc, &write_files, plan_meta, campaign_id, benchmark_tier).await;

        for path in &write_files {
            self.lock_manager.release(path, agent_id);
        }

        MessageGateway::publish_task_completed(&self.bulletin, &self.message_bus, &self.event_bus, task_id, agent_id, session_id, audit_report);
