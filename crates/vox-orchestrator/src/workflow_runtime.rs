...
/// Error type for interpreted workflow execution.
#[derive(Debug, thiserror::Error)]
pub enum WorkflowError {
    /// The requested workflow name was not found in the HIR module.
    #[error("Workflow not found: {0}")]
    NotFound(String),
    /// A runtime error occurred during interpretation (e.g., type mismatch).
    #[error("Interpretation error: {0}")]
    InterpretError(String),
    /// An error occurred during mesh control-plane interaction.
    #[error("Mesh error: {0}")]
    MeshError(String),
}
...
mod tests {
    ...
    /// Without the `mesh` feature, mesh activities emit `MeshActivitySkipped` instead.
    #[tokio::test]
    async fn interpret_workflow_journal_local_and_mesh_no_feature() {
        let body = vec![
            HirStmt::Expr {
                expr: HirExpr::Call(
                    Box::new(HirExpr::Ident("local_step".to_string(), span())),
        ...
        let events: Vec<_> = journal
            ...
            .filter_map(|v| v.get("event").and_then(|e| e.as_str()))
            .collect();
        assert!(events.contains(&"WorkflowStarted"), "missing WorkflowStarted");
        assert!(events.contains(&"ActivityStarted"), "missing ActivityStarted");
        assert!(events.contains(&"ActivityCompleted"), "missing ActivityCompleted");
        assert!(events.contains(&"LocalActivity"), "missing LocalActivity");
        // Without mesh feature, mesh steps are skipped not executed.
        #[cfg(not(feature = "mesh"))]
        assert!(
            events.contains(&"MeshActivitySkipped"),
            "expected MeshActivitySkipped without mesh feature, got: {events:?}"
        );
        assert!(events.contains(&"WorkflowCompleted"), "missing WorkflowCompleted");
    }
}
