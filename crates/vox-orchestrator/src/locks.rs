...
pub async fn release_distributed_lock_with_breaker(
    ...
    db.breaker()
        .call(|| async {
            release_distributed_lock(db.store(), lock_key, node_id, repository_id).await
        })
        .await
}
...
pub async fn prune_stale_distributed_locks_with_breaker(
    ...
    db.breaker()
        .call(|| async { prune_stale_distributed_locks(db.store()).await })
        .await
}
...
pub async fn prune_stale_distributed_locks(store: &vox_pm::store::CodeStore) -> Result<u64, String> {
