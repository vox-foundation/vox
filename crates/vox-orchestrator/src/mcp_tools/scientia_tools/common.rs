...
pub fn operator_publisher_config(
    ...
    if cfg.forge_rest_base.is_none() {
        if let Some(ref b) = news.github_rest_base {
            let b_str: &str = b.as_str();
            let t: &str = b_str.trim();
            if !t.is_empty() {
                cfg.forge_rest_base = Some(String::from(t));
            }
        }
    }
    if cfg.forge_graphql_url.is_none() {
        if let Some(ref u) = news.github_graphql_url {
            let u_str: &str = u.as_str();
            let t: &str = u_str.trim();
            if !t.is_empty() {
                cfg.forge_graphql_url = Some(String::from(t));
            }
        }
    }
    if cfg.opencollective_graphql_url.is_none() {
        if let Some(ref u) = news.opencollective_graphql_url {
            ...
            let t: &str = u_str.trim();
            if !t.is_empty() {
                cfg.opencollective_graphql_url = Some(String::from(t));
            }
        }
    }
