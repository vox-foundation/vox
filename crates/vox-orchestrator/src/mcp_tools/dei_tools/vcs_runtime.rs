...
/// Submit a task through the orchestrator (simpler shape than [`crate::mcp_tools::params::SubmitTaskParams`]).
pub async fn submit_task(state: &ServerState, params: SubmitTaskParams) -> String {
    ...
    let affinities = params
        ...
        .unwrap_or_default()
        .into_iter()
        .map(crate::FileAffinity::write)
        .collect();
    let _agent_id = params.agent_id.map(crate::AgentId);
...
pub async fn heartbeat(state: &ServerState, params: HeartbeatParams) -> String {
    ...
    for agent in orch.status().agents {
        if agent.agent_session_id.as_deref() == Some(params.session_id.as_str()) {
            target = Some(crate::AgentId(agent.id.0));
            break;
        }
    }
    ...
    if let Some(id) = target {
        ...
        orch.event_bus()
            .emit(crate::AgentEventKind::AgentBusy { agent_id: id });
...
pub async fn record_cost(state: &ServerState, params: RecordCostParams) -> String {
    let (target_id, event_bus): (Option<crate::AgentId>, crate::events::EventBus) = {
        let orch = &state.orchestrator;
    ...
    if let Some(id) = target_id {
        ...
        event_bus.emit(crate::AgentEventKind::CostIncurred {
            agent_id: id,
