...
pub async fn generate_vox_code(state: &ServerState, args: serde_json::Value) -> String {
    ...
    loop {
        ...
        let (mut completion, _, _) = match crate::mcp_tools::llm_bridge::mcp_infer_completion(
            state,
            model,
            "vox_generate_code",
            &system_prompt,
...
pub async fn apply_structured_edit(state: &ServerState, args: serde_json::Value) -> String {
    ...
    loop {
        ...
        .await
        {
            ...
            if let Ok((completion, _, _)) = crate::mcp_tools::llm_bridge::mcp_infer_completion(
                state,
                model,
