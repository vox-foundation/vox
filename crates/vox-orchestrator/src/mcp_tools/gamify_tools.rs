...
pub struct AgentHandoffParams {
    ...
    pub to_agent_id: u64,
    /// High-level narrative inserted into [`crate::handoff::HandoffPayload`].
...
/// Emit a [`crate::handoff::HandoffPayload`] (side effect: event bus + downstream listeners).
pub async fn agent_handoff(state: &ServerState, params: AgentHandoffParams) -> String {
    let orch = &state.orchestrator;
    let mut payload = crate::handoff::HandoffPayload::new(
        crate::AgentId(params.from_agent_id),
        Some(crate::AgentId(params.to_agent_id)),
        &params.plan_summary,
    );
    ...
    if let Some(context_json) = params
        ...
        .filter(|s| !s.is_empty())
    {
        let env = match serde_json::from_str::<crate::ContextEnvelope>(context_json) {
            Ok(e) => e,
            Err(err) => {
                return ToolResult::<String>::err_with_remediation(
        ...
        if let Err(e) = crate::context_lifecycle::apply_context_lifecycle_policy(
            ...
            &env,
            expectations,
            crate::context_lifecycle::ContextIngestSource::McpHandoffTool,
        ) {
            return ToolResult::<String>::err_with_remediation(
        ...
        payload.metadata.push((
            crate::handoff::CONTEXT_ENVELOPE_JSON_METADATA_KEY.to_string(),
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        payload.metadata.push((
            crate::handoff::HARNESS_SPEC_JSON_METADATA_KEY.to_string(),
            normalized,
        ));
    }
    if let Err(e) = crate::handoff::execute_handoff(&payload, orch.event_bus()) {
