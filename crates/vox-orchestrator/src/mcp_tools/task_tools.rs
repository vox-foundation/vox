...
fn parse_benchmark_tier(raw: &str) -> Option<crate::ReconstructionBenchmarkTier> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "issue_repair" => Some(crate::ReconstructionBenchmarkTier::IssueRepair),
...
fn enqueue_hints_from_submit_params(params: &SubmitTaskParams) -> Option<TaskEnqueueHints> {
    ...
    let execution_role = match category {
        Some(TaskCategory::Parsing) => Some(crate::AgentExecutionRole::Builder),
        Some(TaskCategory::TypeChecking) => Some(crate::AgentExecutionRole::Builder),
        Some(TaskCategory::Research) => Some(crate::AgentExecutionRole::Researcher),
        Some(TaskCategory::Testing) => Some(crate::AgentExecutionRole::Verifier),
        Some(TaskCategory::Review) => Some(crate::AgentExecutionRole::Verifier),
        Some(TaskCategory::Debugging) => Some(crate::AgentExecutionRole::Reproducer),
        Some(TaskCategory::CodeGen) => Some(crate::AgentExecutionRole::Builder),
        Some(TaskCategory::General) | Some(TaskCategory::Ars) | Some(TaskCategory::Planning) => {
            Some(crate::AgentExecutionRole::Planner)
        }
        _ => None,
    };
...
mod tests {
    ...
    fn enqueue_hints_from_submit_params_maps_testing_category_to_verifier_role() {
        ...
        assert_eq!(
            ...
            Some(crate::AgentExecutionRole::Verifier)
        );
    }
    ...
    fn enqueue_hints_from_submit_params_merges_campaign_tokens_from_description() {
        ...
        assert_eq!(
            ...
            Some(crate::ReconstructionBenchmarkTier::IssueRepair)
        );
...
fn socrates_context_from_retrieval(
    retrieval: &crate::mcp_tools::memory::RetrievalEvidenceEnvelope,
) -> crate::SocratesTaskContext {
    crate::SessionRetrievalEnvelope {
...
pub async fn submit_task(state: &ServerState, params: SubmitTaskParams) -> String {
    ...
    let explicit_context_envelope = match params
        ...
        .filter(|s| !s.is_empty())
    {
        Some(raw) => match serde_json::from_str::<crate::ContextEnvelope>(raw) {
    ...
    let explicit_harness_spec = match params
        ...
        .filter(|s| !s.is_empty())
    {
        Some(raw) => match serde_json::from_str::<crate::AgentHarnessSpec>(raw) {
            Ok(mut harness) => {
    ...
    if state.orchestrator_config.attention_enabled {
        ...
        let att_snap = {
            let g = crate::sync_lock::rw_read(&*bm);
        ...
        let write_file_count = params
            .files
            .iter()
            .filter(|f| matches!(f.access, crate::mcp_tools::params::FileAccess::Write))
            .count();
    ...
    let manifest: Vec<FileAffinity> = params
        ...
        .map(|f| match f.access {
            crate::mcp_tools::params::FileAccess::Write => FileAffinity::write(&f.path),
            crate::mcp_tools::params::FileAccess::Read => FileAffinity::read(&f.path),
        })
        .collect();
    ...
    let planning_mode = params.planning_mode.as_deref().and_then(|m| match m {
        "auto" => Some(crate::PlanningMode::Auto),
        "direct" => Some(crate::PlanningMode::Direct),
        "force_plan" => Some(crate::PlanningMode::ForcePlan),
        "workflow_only" => Some(crate::PlanningMode::WorkflowOnly),
        _ => None,
    });
    ...
    let session_context_to_store: Option<crate::ContextEnvelope> = if let Some(sid) =
        normalized_session_id
            .as_deref()
            .map(str::trim)
            .filter(|s| !s.is_empty())
    {
        let base: Option<crate::ContextEnvelope> =
        ...
        if let Some(mut base) = base {
            ...
            if let Err(e) = crate::context_lifecycle::apply_context_lifecycle_policy(
                ...
                &base,
                ingest_expectations,
                crate::context_lifecycle::ContextIngestSource::McpSubmitTask,
            ) {
                return ToolResult::<SubmitTaskResponse>::err_with_remediation(
    ...
    match submit_result {
        Ok(task_id) => {
            if let Some(ref merged_env) = session_context_to_store {
                if let Some(env) =
                    crate::SessionRetrievalEnvelope::from_context_envelope(merged_env)
                {
                    ...
                    .filter(|s| !s.is_empty())
                {
                    ...
                    if let Ok(raw) = serde_json::to_string(merged) {
                        ...
                        match crate::mcp_tools::sync_poison::poison_rw_write(
                            ...
                            Ok(mut ctx) => {
                                ctx.set(crate::AgentId(0), &context_key, raw, 3600);
                            }
                            Err(e) => {
                                tracing::warn!(
                                    error = %e,
            ...
            {
                let pseudo = vec![crate::mcp_tools::chat_tools::params::PlanTask {
                    id: 1,
                ...
                let rep = crate::mcp_tools::chat_tools::analyze_plan_gaps(
                    &description,
                    params.files.len(),
                    router_hint,
                    None,
                ...
                Some(crate::mcp_tools::params::SubmitShadowAdequacy {
...
pub async fn complete_task(state: &ServerState, params: CompleteTaskParams) -> String {
    ...
    let attestation = crate::CompletionAttestation {
...
pub async fn fail_task(state: &ServerState, params: FailTaskParams) -> String {
    let task_id = TaskId(params.task_id);
    let assigned = state.orchestrator.agent_assigned_to_task(task_id);
    let res = state.orchestrator.fail_task(task_id, params.reason).map_err(|e| e.to_string());
...
pub async fn reorder_task(state: &ServerState, params: crate::mcp_tools::params::ReorderTaskParams) -> String {
    ...
    match state
        .orchestrator.reorder_task(TaskId(params.task_id), priority)
        .await
    {
...
pub async fn drain_agent(state: &ServerState, params: DrainAgentParams) -> String {
    match state.orchestrator.drain_agent(AgentId(params.agent_id)).map_err(|e| e.to_string()) {
