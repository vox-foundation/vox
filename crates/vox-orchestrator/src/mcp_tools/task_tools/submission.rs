...
pub fn parse_benchmark_tier(raw: &str) -> Option<crate::ReconstructionBenchmarkTier> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "issue_repair" => Some(crate::ReconstructionBenchmarkTier::IssueRepair),
        "subsystem_regen" => Some(crate::ReconstructionBenchmarkTier::SubsystemRegen),
        "crate_regen" => Some(crate::ReconstructionBenchmarkTier::CrateRegen),
...
pub async fn submit_task(state: &ServerState, params: SubmitTaskParams) -> String {
    ...
    let manifest: Vec<crate::FileAffinity> = params
        .files
        .iter()
        .map(|f| match f.access {
            crate::mcp_tools::params::FileAccess::Write => crate::FileAffinity::write(&f.path),
            crate::mcp_tools::params::FileAccess::Read => crate::FileAffinity::read(&f.path),
        })
        .collect();
    ...
    let submit_result: Result<crate::TaskId, String> = if params.planning_mode.is_some() {
        orch.submit_goal(
            description.clone(),
            manifest,
            priority,
    ...
    match submit_result {
        Ok(task_id) => {
            ...
            if let Some((_, Some(ref w), _)) = canonical_info {
                if !w.is_empty() {
                    orch.event_bus()
                        .emit(crate::AgentEventKind::PromptConflictDetected {
                            task_id,
                            warnings: w.clone(),
                        });
                }
