use crate::mcp_tools::server_state::ServerState;
use crate::mcp_tools::params::ToolResult;

const REM_BUDGET_LOCK: &str = "Retry; persistent poisoned-lock errors usually need an MCP restart.";

/// Get current context window usage and compaction recommendation (async).
pub async fn compaction_status(
    state: &ServerState,
    params: crate::mcp_tools::mcp_context::ContextBudgetParams,
) -> String {
    let orch = &state.orchestrator;
    let id = crate::AgentId(params.agent_id);
    let handle = orch.budget_handle();
    let budget_lock = match crate::mcp_tools::sync_poison::poison_rw_read(handle.read(), "agent budget") {
        Ok(g) => g,
        Err(e) => {
            return ToolResult::<String>::err_with_remediation(e.to_string(), REM_BUDGET_LOCK)
                .to_json();
        }
    };
    if let Some(budget) = budget_lock.check_budget(id) {
        let engine = crate::CompactionEngine::default();
        let should = engine.should_compact(budget.tokens_used);
        ToolResult::ok(format!(
            "Agent {}: {}/{} tokens used. Compaction recommended: {}. Strategy: {}",
            params.agent_id,
            budget.tokens_used,
            budget.model_max_tokens,
            should,
            crate::CompactionStrategy::default()
        ))
        .to_json()
    } else {
        ToolResult::ok(format!(
            "Agent {}: no budget tracked. Compaction engine ready with {}k token limit.",
            params.agent_id,
            crate::CompactionConfig::default().max_context_tokens / 1000
        ))
        .to_json()
    }
}

