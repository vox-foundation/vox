//! Local JSON file queue for optional telemetry upload (`vox telemetry upload`).
//!
//! Layout under the spool root: `pending/<uuid>.json` (one JSON object per file). See ADR 023 and
...
pub trait AtomicAppend {
    fn append_sync(&self, value: &impl Serialize) -> Result<PathBuf>;
}
...
impl AtomicAppend for Spooler {
    fn append_sync(&self, value: &impl Serialize) -> Result<PathBuf> {
        enqueue(&self.root, value)
    }
}
...
/// Resolved spool directory (creates `pending/` on demand).
#[must_use]
