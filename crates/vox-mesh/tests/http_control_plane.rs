...
use vox_mesh::transport::{MeshHttpAuth, MeshTransportState};
use vox_mesh::{node_record_for_current_process, transport};
...
async fn mesh_token_requires_bearer() {
    ...
    let app = transport::mesh_http_app_with_auth(
        state,
        MeshHttpAuth::Bearer("unit-test-mesh-token".into()),
    );
    let server = tokio::spawn(async move {
        axum::serve(listener, app).await.unwrap();
    });
    ...
    let authed = MeshHttpClient::new(&base).with_bearer("unit-test-mesh-token");
...
async fn health_ok_without_bearer_when_token_required() {
    ...
    let app = transport::mesh_http_app_with_auth(
        state,
        transport::MeshHttpAuth::Bearer("unit-test-mesh-token".into()),
    );
    let server = tokio::spawn(async move {
        axum::serve(listener, app).await.unwrap();
    });
