//! Minimal HTTP control plane for mesh join / list / heartbeat / leave (loopback-first).
//!
...
//!
//! When **`VOX_MESH_SCOPE_ID`** is set on the server process, **`POST /v1/mesh/join`** and
//! **`POST /v1/mesh/heartbeat`** require the JSON [`crate::NodeRecord::scope_id`] to match.
...
pub struct MeshTransportState {
    inner: Arc<RwLock<MeshRegistryFile>>,
...
impl MeshTransportState {
    ...
    pub fn with_required_scope(scope: Option<String>) -> Self {
        ...
        Self {
            inner: Arc::new(RwLock::new(MeshRegistryFile {
                schema_version: 1,
                nodes: Vec::new(),
            })),
            required_scope,
...
async fn list_nodes(State(st): State<MeshTransportState>) -> Json<MeshRegistryFile> {
...
async fn join_node(
    ...
    if !scope_ok(&st, &node) {
        return Err(ResponseErr(
            ...
            "mesh scope mismatch: set VOX_MESH_SCOPE_ID to match server",
        ));
    }
...
pub fn router(state: MeshTransportState) -> Router {
    Router::new()
        ...
        .route("/v1/mesh/nodes", get(list_nodes))
...
pub fn mesh_http_app_with_auth(state: MeshTransportState, auth: MeshHttpAuth) -> Router {
    ...
    let expected: Option<Arc<str>> = match auth {
        MeshHttpAuth::FromEnv => mesh_control_token_from_env().map(Arc::from),
...
pub async fn serve(addr: SocketAddr, state: MeshTransportState) -> Result<(), std::io::Error> {
    ...
    info!(%addr, "vox-mesh HTTP control plane listening");
