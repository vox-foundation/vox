//! Vox **mesh** — node registry and (optional) HTTP control plane.
//!
//! CPU-first: each [`NodeRecord`] carries [`vox_orchestrator::TaskCapabilityHints`]. See
//! `docs/src/architecture/mesh-ssot.md` for environment variables.
...
/// Parsed mesh-related environment (best-effort).
...
pub struct MeshEnv {
    ...
    /// `VOX_MESH_SCOPE_ID` — mesh cluster / tenancy id (join/heartbeat must match server when server enforces scope).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub scope_id: Option<String>,
}

/// Merge `Vox.toml` `[mesh]` into env-derived values when the corresponding env is unset.
...
pub fn mesh_env_resolved(vox_toml_path: Option<&std::path::Path>) -> MeshEnv {
    ...
    let Ok(Some(toml)) = vox_repository::read_vox_mesh_toml(path) else {
        return env;
    };
...
/// Whether `VOX_MESH_ADVERTISE_GPU` is set, or `[mesh].advertise_gpu = true` when env is unset.
#[must_use]
pub fn mesh_advertise_gpu_effective(vox_toml_path: Option<&std::path::Path>) -> bool {
    ...
    matches!(
        vox_repository::read_vox_mesh_toml(path),
...
/// the value is meant for `vox mesh serve --bind`, not outbound requests.
#[must_use]
...
/// Read mesh env vars (does not mutate process state).
#[must_use]
...
/// One participant in the mesh view.
...
pub struct NodeRecord {
    ...
    /// `CARGO_PKG_VERSION` of `vox-mesh` / embedding crate at registration time.
    pub version: String,
    /// Wall-clock last update (epoch ms).
    pub last_seen_unix_ms: u64,
    /// Mesh tenancy / cluster id; must match server [`crate::transport::MeshTransportState::required_scope`] when the server enforces scope.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub scope_id: Option<String>,
}

/// Serializable registry file (`.vox/cache/mesh/local-registry.json`).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MeshRegistryFile {
...
impl LocalRegistry {
    /// Default path under the user home: `~/.vox/cache/mesh/local-registry.json`.
    #[must_use]
...
/// Build a [`NodeRecord`] for this process using orchestrator host probe + mesh env labels.
#[must_use]
...
mod normalize_http_control_base_tests {
    ...
    fn adds_scheme_and_strips_slash() {
        assert_eq!(
            normalize_http_control_base("mesh-ctrl:9847/").as_deref(),
            Some("http://mesh-ctrl:9847")
        );
    }
