...
fn populi_middleware_client_builds_with_and_without_retry() {
    ...
    let _no_retry = vox_reqwest_defaults::populi_control_plane_client(inner.clone(), false);
    let _with_retry = vox_reqwest_defaults::populi_control_plane_client(inner, true);
}
