...
async fn submit_returns_classified_priority() {
    ...
    assert!(["urgent", "normal", "background"].contains(
        &v["data"]["classified_priority"].as_str().unwrap()
    ));
...
async fn inbox_lists_submitted_items() {
    ...
    hopper.submit("task A".into(), vec![], PriorityHint::Normal, IntakeSource::Developer, None).await;
    hopper.submit("task B".into(), vec![], PriorityHint::Urgent, IntakeSource::Agent, None).await;
...
async fn reprioritize_round_trip_with_audit_trail() {
    ...
    let item = hopper.submit(
        "improve perf".into(), vec![], PriorityHint::Normal,
    ...
    assert!(v["data"]["audit_id"].as_str().unwrap().starts_with("audit-"));
...
async fn reprioritize_without_confirm_token_returns_400() {
    ...
    use vox_orchestrator::hopper::{HopperIntake, IntakeSource, PriorityHint};
    let item = hopper.submit(
        "task".into(), vec![], PriorityHint::Normal, IntakeSource::Developer, None,
    ).await;
    ...
    let res = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!("/api/v2/hopper/items/{}/reprioritize", item.item_id.0))
                .header("content-type", "application/json")
                .body(json_body(serde_json::json!({
                    "new_priority": "urgent",
                    "reason": "forgot token"
