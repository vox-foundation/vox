...
pub const TOOL_REGISTRY: &[(&str, &str)] = &[
    ...
    (
        "vox_config_set",
        "Update the orchestrator configuration dynamically.",
    ),
    (
    ...
    (
        ...
        "Set sticky MCP chat model id for chat / inline / ghost.",
    ),
    (
        "vox_get_active_model",
        "Show sticky MCP chat override and resolved ModelSpec.",
    ),
    (
        "vox_oratio_transcribe",
        "Transcribe audio to text via Vox Oratio (Candle Whisper).",
    ),
    (
        "vox_oratio_status",
        "Oratio / Candle Whisper backend status and default model env.",
    ),
    (
        "vox_chat_message",
        "Send a chat message to the Vox AI. Resolves @mentions, injects editor context, queries LLM.",
    ),
    (
        "vox_chat_history",
    ...
    (
        "vox_plan",
        "Generate a Cursor-style structured task plan for a goal.",
    ),
    (
        "vox_replan",
        "Replan via vox-dei-d: session_id + delta_hint.",
    ),
    (
        "vox_plan_status",
        "Plan session status via vox-dei-d for a session_id.",
    ),
    (
        "vox_benchmark_list",
        "List recent benchmark_event rows from Codex for this repository.",
    ),
    (
        "vox_schola_submit",
        "Enqueue a background orchestrator task for Mens training intent.",
    ),
    (
        "vox_populi_local_status",
        "Return mens environment variables and the local mens registry file contents.",
    ),
    (
        "vox_reliability_list",
        "List ranked LLM endpoint reliability metrics (hallucination rates, failures, timeouts).",
    ),
    (
        "vox_reliability_agents",
