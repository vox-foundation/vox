//! OpenAI-style tool definitions and execution for Populi chat.
//!
...
fn direct_populi_chat_capabilities(
    registry: &CapabilityRegistry,
) -> impl Iterator<Item = &CapabilityDescriptor> + '_ {
    registry
        .populi_chat_capabilities()
...
pub fn chat_tool_definitions() -> Vec<serde_json::Value> {
    ...
    direct_populi_chat_capabilities(&registry)
        .map(|cap| {
            let name = mcp_tool_name(cap);
            let params = populi_chat_parameters(&cap.capability_id);
...
pub fn execute_tool_calls(tool_calls: &[ToolCall]) -> Vec<(String, String)> {
    ...
    let registry = default_registry();
    let allowed: std::collections::HashSet<String> = direct_populi_chat_capabilities(&registry)
