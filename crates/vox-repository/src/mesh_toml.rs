//! Optional `[mesh]` section in workspace `Vox.toml` (operator SSOT; env overrides apply in consumers).
...
/// Parsed `[mesh]` table from `Vox.toml`.
#[derive(Debug, Clone, Default, Deserialize, PartialEq, Eq)]
#[serde(default)]
pub struct VoxMeshToml {
    /// HTTP control plane base URL (`GET /v1/mesh/nodes`); maps to `OrchestratorConfig::mesh_control_url`.
    #[serde(default)]
...
/// Read `[mesh]` from `path` (typically `.../Vox.toml`). Returns `None` if the file or section is missing.
pub fn read_vox_mesh_toml(path: &Path) -> Result<Option<VoxMeshToml>, VoxMeshTomlError> {
    if !path.is_file() {
        return Ok(None);
    }
    let content = std::fs::read_to_string(path)?;
    let table: toml::Table = toml::from_str(&content)?;
    let Some(mesh_val) = table.get("mesh") else {
        return Ok(None);
    };
    let section_str = toml::to_string(mesh_val)?;
    let mesh: VoxMeshToml = toml::from_str(&section_str)?;
    let labels_empty = match mesh.labels.as_ref() {
        None => true,
        Some(labels) => labels.is_empty(),
    };
    if mesh.control_url.is_none()
        && mesh.scope_id.is_none()
        && mesh.advertise_gpu.is_none()
        && labels_empty
    {
        return Ok(None);
    }
...
"#,
        ...
        .unwrap();
        let m = read_vox_mesh_toml(&p).unwrap().expect("some mesh");
    ...
    fn missing_file_returns_none() {
        ...
        assert!(read_vox_mesh_toml(p).unwrap().is_none());
    }
}
