...
//!
//! Copied verbatim from `vox-populi/src/mens/tensor/hf_load.rs` (SP3 sub-batch B).
//! `use vox_bounded_fs::read_utf8_path_capped` replaced with an inline `std::fs::read_to_string`
//! to avoid pulling `vox-scaling-policy` into the plugin's dependency graph.
