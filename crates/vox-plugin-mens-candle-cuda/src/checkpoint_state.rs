...
//!
//! Copied verbatim from `vox-populi/src/mens/tensor/checkpoint_state.rs` (SP3 sub-batch A).
//! Pure-data — only depends on `anyhow`, `serde`, `serde_json`, `tracing`, and `std`.
//! All of these are already present in this crate's Cargo.toml.
//!
