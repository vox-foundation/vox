//! `vox-train status` — show training run status from telemetry log.
...
pub fn run(run_dir: Option<PathBuf>) -> Result<()> {
    let dir = run_dir.unwrap_or_else(|| PathBuf::from("populi/runs/latest"));
    let log = dir.join("telemetry_log.jsonl");
    if !log.is_file() {
        println!("No telemetry log found at {}", log.display());
        println!("Start training with: vox-train train --model <HF_REPO>");
        return Ok(());
    }
    ...
    if ckpt.is_file() {
        ...
        println!("  Resume with: vox-train train --resume {}", dir.display());
    }

    let adapter = dir.join("candle_qlora_adapter.safetensors");
    if adapter.is_file() {
        println!("\nFinal adapter: {}", adapter.display());
        println!("  Serve with: vox-train serve --model {}", dir.display());
    }
