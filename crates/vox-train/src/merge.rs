//! `vox-train merge` — merge a QLoRA adapter into base weights.
...
pub fn run(args: Args) -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!("{}", vox_populi::MERGE_QLORA_REJECTS_BURN_BIN);
    }
    if !meta.is_file() {
    ...
    let meta_v2 = if let Ok(m) =
        serde_json::from_str::<vox_populi::tensor::candle_qlora_merge::QloraAdapterMetaV2>(&raw)
    {
        m
    } else {
        let v3: vox_populi::tensor::adapter_schema_v3::PopuliAdapterManifestV3 =
            serde_json::from_str(&raw)
                .with_context(|| format!("parse meta as v2 or v3 {}", meta.display()))?;
        vox_populi::tensor::adapter_schema_v3::to_qlora_meta_v2_for_merge(&v3)
            .with_context(|| "adapter manifest v3 → merge bridge")?
    };
    vox_populi::tensor::candle_qlora_merge::merge_qlora_v2_into_base_subset(
        &base_shard,
        &adapter,
        &meta_v2,
        &output,
