//! CLI argument parsing and dispatch for `vox-train`.
...
#[command(
    ...
    long_about = "Standalone GPU-native binary for the Vox Populi ML subsystem.\n\
                  Equivalent to `vox populi …` but ~3× faster to compile (no lexer/parser/codegen).\n\n\
                  Quick start:\n\
                  \n  vox-train train --model Qwen/Qwen2.5-Coder-1.5B-Instruct\
                  \n  vox-train serve --model populi/runs/latest\
                  \n  vox-train probe",
    version
)]
pub struct Args {
...
pub enum Cmd {
    ...
    Status {
        /// Run directory (default: populi/runs/latest).
        #[arg(long)]
