//! `vox-train train` — dispatches QLoRA training.
...
pub async fn run(args: Args) -> Result<()> {
    ...
    let device_profile = vox_populi::DeviceProfile::from_gpu_info(&gpu_info.model_name, gpu_info.vram_mb);
    ...
    let mix_config = workspace_root.as_ref().map(|r| r.join("populi/config/mix.yaml"));
    ...
    if let Some(ref repo_id) = model {
        ...
        match rx.recv().map_err(|_| anyhow::anyhow!("HF download thread exited"))? {
            Ok(files) if files.is_safetensors() => {
                ...
                if let Ok(arch) = vox_populi::tensor::hf_load::detect_hf_architecture(&files.config) {
                    let cfg = vox_populi::tensor::hf_load::config_dims_for_architecture(&files.config, arch)
                        .map_err(|e| anyhow::anyhow!("HF config: {}", e))?;
                    let est_mb = vox_populi::estimate_training_vram_mb_qlora(
    ...
    vox_populi::run_populi_training(
        vox_populi::PopuliTrainBackend::CandleQlora,
        &data_dir,
        Some(&output_dir),
        &config,
        device_kind,
...
fn spawn_background(log_dir: Option<PathBuf>) -> Result<()> {
    let log_dir = log_dir.unwrap_or_else(|| PathBuf::from("populi/runs/logs"));
