...
pub async fn probe_mastodon(cfg: &PublisherConfig) -> HeartbeatStatus {
    ...
    let instance_url = if domain.starts_with("http") { domain.to_string() } else { format!("https://{}", domain) };
...
pub async fn probe_bluesky(_cfg: &PublisherConfig) -> HeartbeatStatus {
    ...
    let client = Client::new();
    let start = Instant::now();
    let url = format!("{}/xrpc/com.atproto.server.describeServer", pds_base.trim_end_matches('/'));
    let res = client.get(&url).send().await;
...
pub async fn probe_discord(_cfg: &PublisherConfig) -> HeartbeatStatus {
    let webhook_url = match vox_clavis::resolve_secret(vox_clavis::SecretId::VoxSocialDiscordWebhook).expose() {
        Some(s) => s.to_string(),
        None => return HeartbeatStatus::AuthFailure { hint: "Missing Discord Webhook Secret".to_string() },
    };
...
pub async fn probe_twitter(cfg: &PublisherConfig) -> HeartbeatStatus {
    let token = match cfg.twitter_bearer_token.clone().or_else(|| {
        vox_clavis::resolve_secret(vox_clavis::SecretId::VoxNewsTwitterBearer).expose().map(String::from)
    }) {
        Some(t) => t,
        None => return HeartbeatStatus::AuthFailure { hint: "Missing Twitter Bearer Token".to_string() },
    };

    let client = Client::new();
    let start = Instant::now();
    let base = cfg.twitter_api_base.as_deref().unwrap_or("https://api.twitter.com");
...
pub async fn probe_linkedin(cfg: &PublisherConfig) -> HeartbeatStatus {
    let token = match cfg.linkedin_access_token.clone().or_else(|| {
        vox_clavis::resolve_secret(vox_clavis::SecretId::VoxSocialLinkedinAccessToken).expose().map(String::from)
    }) {
        Some(t) => t,
        None => return HeartbeatStatus::AuthFailure { hint: "Missing LinkedIn Access Token".to_string() },
    };

    let client = Client::new();
    let start = Instant::now();
    let base = cfg.linkedin_api_base.as_deref().unwrap_or("https://api.linkedin.com");
...
pub async fn probe_zenodo(_cfg: &PublisherConfig) -> HeartbeatStatus {
    ...
    let token = match secret.expose() {
        Some(t) => t,
        None => return HeartbeatStatus::AuthFailure { hint: "Missing Zenodo Access Token".to_string() },
    };
    ...
    let res = client.get(url).query(&[("access_token", token)]).send().await;
    map_res(res, start)
}
...
pub async fn probe_opencollective(cfg: &PublisherConfig) -> HeartbeatStatus {
    let token = match cfg.open_collective_token.clone().or_else(|| {
        vox_clavis::resolve_secret(vox_clavis::SecretId::VoxNewsOpenCollectiveToken).expose().map(String::from)
    }) {
        Some(t) => t,
        None => return HeartbeatStatus::AuthFailure { hint: "Missing OpenCollective Token".to_string() },
    };

    let client = Client::new();
    let start = Instant::now();
    let url = cfg.opencollective_graphql_url.as_deref().unwrap_or("https://api.opencollective.com/graphql/v2");
