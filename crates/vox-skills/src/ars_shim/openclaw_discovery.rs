...
) -> Result<(OpenClawResolvedEndpoints, u64), crate::ars_shim::openclaw_adapter::OpenClawAdapterError>
{
    let client = vox_reqwest_defaults::client_builder()
        ...
        .build()
        .map_err(|e| {
            crate::ars_shim::openclaw_adapter::OpenClawAdapterError::Other(e.to_string())
        })?;
    let resp = client.get(well_known_url).send().await.map_err(|e| {
        crate::ars_shim::openclaw_adapter::OpenClawAdapterError::Other(e.to_string())
    })?;
    if !resp.status().is_success() {
        return Err(
            crate::ars_shim::openclaw_adapter::OpenClawAdapterError::Other(format!(
                "OpenClaw discovery HTTP {}",
                resp.status()
            )),
        );
    }
    let doc: OpenClawWellKnownDocument = resp.json().await.map_err(|e| {
        crate::ars_shim::openclaw_adapter::OpenClawAdapterError::Other(e.to_string())
    })?;
