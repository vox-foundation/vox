//! AudioCapture implementation for Oratio microphone device backend.
//!
//! This plugin handles the low-level microphone enumeration and PCM capture.
...
//! TODO(SP7-followup): wire CPAL / native audio APIs from vox-oratio mic module.
...
#[derive(Clone)]
pub(crate) struct OratioMicPlugin;

impl VoxPlugin for OratioMicPlugin {
    fn id(&self) -> RString {
        RString::from("oratio-mic")
    }
...
impl AudioCapture for OratioMicPlugin {
    fn list_devices_json(&self) -> RResult<RString, RBoxError> {
        // TODO(SP7-followup): enumerate real microphone devices via CPAL
