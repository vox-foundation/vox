...
/// CLI preset: honor `RUST_LOG` when valid; otherwise default filter **`info`** (matches legacy `vox-cli-core`).
///
/// Duplicate calls are harmless — subsequent [`tracing_subscriber::fmt::Subscriber`] installs are ignored.
...
/// Daemon/service preset: [`EnvFilter::from_default_env`] (unset ⇒ subscriber default levels, typically errors-only).
