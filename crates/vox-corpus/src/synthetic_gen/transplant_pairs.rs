...
pub fn generate_transplant_pairs(input_path: &std::path::Path, out: &mut impl Write, count: usize) -> anyhow::Result<usize> {
    use std::io::BufRead;
    ...
    for _ in 0..count {
        ...
        let source_code = source_v.get("response").and_then(|r| r.as_str()).unwrap_or("");
        let target_code = target_v.get("response").and_then(|r| r.as_str()).unwrap_or("");

        // Very basic transplant: if source has a 'fn' and target has an 'actor' or '@table'
        if source_code.contains("fn ") && (target_code.contains("actor ") || target_code.contains("@table type ")) {
