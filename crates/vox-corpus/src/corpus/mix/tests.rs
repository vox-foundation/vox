...
fn incremental_skip_works() {
    ...
    run_mix_with_options(&cfg_path, None, MixRunOptions { strict: true, write_report: true }).expect("first run");
    assert!(out_path.is_file());
    assert!(dir.path().join("out.mix_report.json").is_file());
    
    // Measure time for second run (should be skip)
    let start = std::time::Instant::now();
    run_mix_with_options(&cfg_path, None, MixRunOptions { strict: true, write_report: true }).expect("second run");
    let elapsed = start.elapsed();
    
    // On a fast system, a skip should be < 10ms for a tiny file, but even on CI it should be very fast.
    assert!(elapsed.as_millis() < 500, "skip took {}ms", elapsed.as_millis());
}
...
fn mix_processes_repeats_and_multiple_sources() {
    ...
    std::fs::write(&s1_path, "{\"lane\":\"vox_codegen\",\"prompt\":\"s1\",\"response\":\"r1\"}\n").unwrap();
    std::fs::write(&s2_path, "{\"lane\":\"vox_codegen\",\"prompt\":\"s2\",\"response\":\"r2\"}\n").unwrap();
    ...
    run_mix_with_options(&cfg_path, None, MixRunOptions { strict: true, write_report: true }).expect("run");
