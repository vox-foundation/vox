...
fn main() {
    ...
    if verify {
        if !target_file.exists() {
            eprintln!("Verification failed: {} does not exist.", target_file.display());
            std::process::exit(1);
        }
        let existing = fs::read_to_string(&target_file).unwrap();
        if existing != code {
            eprintln!("Verification failed: {} is out of sync with generator. Run `cargo run -p vox-jsonschema-util --example emit_agent_harness` to fix.", target_file.display());
            // Optional: print diff
            std::process::exit(1);
        }
