...
#[cfg(feature = "serve")]
async fn transcribe_handler(mut multipart: Multipart) -> Result<Json<crate::backends::asr_backend::AsrOutput>, axum::http::StatusCode> {
    ...
    while let Some(field) = multipart.next_field().await.map_err(|_| axum::http::StatusCode::BAD_REQUEST)? {
        ...
        if name == "file" {
            ...
            let text = field.text().await.map_err(|_| axum::http::StatusCode::BAD_REQUEST)?;
