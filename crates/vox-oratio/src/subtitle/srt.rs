    ...
    for block in parts {
        ...
        if lines.len() >= 3 {
            ...
            if let Some((start_str, end_str)) = time_line.split_once(" --> ") {
                ...
                segments.push(TimedSegment { start_ms, end_ms, text });
            }
        }
    }
    segments
...
pub fn generate_srt_file(
    ...
    if out.segments.is_empty() {
        ...
        if !out.raw_text.trim().is_empty() {
            ...
            let srt = format_srt(&[TimedSegment {
                start_ms: 0,
                end_ms: duration_ms,
                text: out.raw_text,
            ...
            file.write_all(srt.as_bytes()).context("Failed to write SRT file")?;
    ...
    let mut file = File::create(&output_path).context("Failed to create SRT file")?;
    file.write_all(srt_content.as_bytes()).context("Failed to write SRT file")?;
