...
use crate::model::{
    Qwen2Attention, Qwen2MLP, Qwen35AttentionBlock, Qwen35Layer, Qwen35LayerCache,
...
impl InferenceEngine {
    pub fn load(model_dir: &Path, device_kind: &crate::device::DeviceKind) -> Result<Self> {
        ...
        let lora_factors: std::collections::HashMap<String, crate::merge::LoraFactors>;
        ...
        if is_quantized {
            ...
            lora_factors = std::collections::HashMap::new();
        } else {
            ...
            lora_factors = resolved.lora_factors;
        ...
        let linear_weight = |key: &str| -> Result<Tensor> {
            ...
            if is_quantized {
                Ok(Tensor::zeros((1,), candle_core::DType::F32, &_device)?)
            } else {
                fold_lora_delta(get_tensor(key)?, key, &lora_factors)
            }
        };

        // Helper to find a linear layer as native QMatMul or QuantizedLinear
        let get_linear = |key: &str, unquantized_weight: &Tensor| -> Result<QuantizedLinear> {
            if is_quantized {
                let qmeta = quant_meta.as_ref().unwrap();
                let tm = qmeta
                    .tensors
                    ...
                    .or_else(|| {
                        if key == "lm_head.weight" {
                            qmeta
                                ...
                                .or_else(|| qmeta.tensors.get("model.embed_tokens.weight"))
                        } else {
                            None
                        }
                    })
                    .ok_or_else(|| anyhow::anyhow!("Tensor `{key}` not in quant-metadata.json"))?;
                for st in &weight_maps {
                    let view = st.tensor(key).or_else(|_| {
                        if key == "lm_head.weight" {
                            st.tensor("model.language_model.embed_tokens.weight")
                                .or_else(|_| st.tensor("model.embed_tokens.weight"))
                        } else {
                            st.tensor(key)
                        }
                    });
...
mod tests {
    ...
    fn test_rope_inv_freq_synthesis_qwen_default() {
        let dev = candle_core::Device::Cpu;
        let t = super::synthesize_rope_inv_freq(128, Some(10_000_000.0), &dev).expect("synthesize");
        assert_eq!(t.dims(), &[64]);
