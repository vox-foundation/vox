...
fn codegen_index_emits_ddl() {
    use vox_compiler_emit::codegen_rust::emit::emit_index_ddl;
...
"#;
    ...
    let module = parse(tokens).expect("Should parse");
    let hir = lower_module(&module);
    let output = vox_compiler_emit::codegen_rust::generate(&hir, "my_mcp_tools").unwrap();
...
"#;
    ...
    let mcp = vox_compiler_emit::codegen_rust::emit::emit_mcp_server(&hir, "my_tools");
...
"#;
    ...
    let module = parse(tokens).expect("Should parse");
    let hir = lower_module(&module);
    let output = vox_compiler_emit::codegen_rust::generate(&hir, "test_no_mcp").unwrap();
...
"#;
    ...
    let output = vox_compiler_emit::codegen_rust::generate(&hir, "with_res").unwrap();
...
"#;
    ...
    let module = parse(tokens).expect("Should parse");
    let hir = lower_module(&module);
    let mcp = vox_compiler_emit::codegen_rust::emit::emit_mcp_server(&hir, "lst");
