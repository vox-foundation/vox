...
async fn test_infinite_loop_actor_yields_to_scheduler() {
    ...
    let actor_task = tokio::spawn(async move {
        ...
        let mut ctx = vox_runtime::process::ProcessContext::new(vox_runtime::pid::Pid::new(), mailbox_rx);
    ...
    assert!(result.is_ok(), "Starvation Guard Failed: The infinite loop blocked the Tokio Scheduler.");
