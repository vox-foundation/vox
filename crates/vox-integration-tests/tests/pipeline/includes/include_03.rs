...
fn pipeline_multi_route_rust_codegen() {
    ...
    let module = parse(tokens).unwrap();
    let hir = vox_compiler::hir::lower_module(&module);
    let output = vox_compiler_emit::codegen_rust::generate(&hir, "multi_route_app").unwrap();
...
/// OP-S032: integration gate — AST [`vox_compiler_emit::codegen_ts::jsx`] and HIR [`emit_hir`] share [`compat`] DOM edges.
#[test]
fn pipeline_compat_tag_gate_jsx_hir_emit_matrix() {
    ...
    for (vox, react) in edges {
        assert_eq!(
            vox_compiler_emit::codegen_ts::hir_emit::map_jsx_attr_name(vox),
            react
        );
        assert_eq!(vox_compiler_emit::codegen_ts::jsx::map_jsx_attr_name(vox), react);
    }
}
...
fn pipeline_express_contract_mapper_fixture_validates_multi_route_hir() {
    ...
    let module = parse(tokens).unwrap();
    let hir = vox_compiler::hir::lower_module(&module);
    vox_compiler_emit::codegen_ts::routes::validate_express_route_emit_input(&hir)
...
fn pipeline_route_component_express_and_web_ir_gate() {
    ...
    let module = parse(tokens).unwrap();
    let hir = vox_compiler::hir::lower_module(&module);
    vox_compiler_emit::codegen_ts::routes::validate_express_route_emit_input(&hir).expect("express");
    let (web, summary) =
        vox_compiler_emit::web_ir::lower::lower_hir_to_web_ir_with_summary(&hir);
    assert!(
