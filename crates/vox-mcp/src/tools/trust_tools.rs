...
const REM_VOXDB_TRUST: &str =
    "Attach VoxDb (Turso) to the MCP server via `VOX_DB_PATH` / `VOX_DB_URL` for trust rollup tools.";
const REM_TRUST_ARGS: &str =
    "Use optional string filters (`entity_type`, `dimension`, `domain`, `repository_id`) and numeric bounds.";
const REM_TRUST_SUMMARY_ARGS: &str =
    "Set `group_by` to dimension|domain|entity_type|dimension_domain|entity_dimension; optional `limit_groups` 1..500.";
const REM_TRUST_DRIFT_ARGS: &str =
    "Optional `entity_type`, `dimension`, `window_ms` (60s–30d, default 86400000).";
const REM_TRUST_PROPAGATE_ARGS: &str =
    "`dimension` required; optional `damping` 0–1, `iterations` 1–256, `persist` to write *_propagated observations; set `repository_id_default_workspace` true to scope rollups.";
...
pub async fn trust_observation_drift(state: &ServerState, args: serde_json::Value) -> String {
    ...
    match db
        ...
        .await
    {
        Ok(rep) => ToolResult::ok(serde_json::to_value(&rep).unwrap_or_default()).to_json(),
        Err(e) => ToolResult::<serde_json::Value>::err_with_remediation(
            e.to_string(),
