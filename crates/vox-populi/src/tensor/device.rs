//! Device selection, GPU probing (best-effort), and VRAM heuristics for Populi training.
...
/// Log GPU summary via [`tracing`] (`target = "vox_populi_gpu"`).
...
pub fn oom_guidance() -> &'static str {
    "Try `--preset safe`, lower `--seq-len` / `--batch-size`, or `--device cpu`. See docs/src/architecture/populi-training-ssot.md."
}
