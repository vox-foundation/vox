...
impl ExecutionPlanner {
    ...
    pub fn validate_hard_gates(&self, c: &FineTuneContract) -> anyhow::Result<()> {
        ...
        if c.adapter.method == AdapterMethod::Qlora
            && c.data.tokenizer_mode != PopuliTokenizerMode::Hf
        {
            anyhow::bail!(super::operator_messages::QLORA_REQUIRES_HF_TOKENIZER);
        }

        if c.data.tokenizer_mode == PopuliTokenizerMode::Hf
        ...
        if c.adapter.method == AdapterMethod::Qlora {
            anyhow::bail!(
                "This build does not include the Candle QLoRA kernel (`candle-qlora` feature). Rebuild with `vox-populi` `train` / `candle-qlora`."
            );
        }
...
mod tests {
    ...
    use crate::tensor::finetune_contract::{
        AdapterSpec, AdapterTargetMask, ArtifactSpec, DataSpec, ExecSpec, FineTuneContract,
        ModelSpec, QuantSpec,
    };
    use crate::tensor::training_config::{PopuliTokenizerMode, TrainingDeploymentTarget};
