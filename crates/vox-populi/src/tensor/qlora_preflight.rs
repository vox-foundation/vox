//! HF file checks before native Candle QLoRA (`vox populi train --backend qlora`).
//!
//! **Tokenizer contract:** `--backend qlora` requires [`PopuliTokenizerMode::Hf`] and a real
...
//! `candle_qlora_train.rs` and mdBook [`populi-training-ssot.md`](../../../../docs/src/architecture/populi-training-ssot.md).
...
use super::training_config::{LoraTrainingConfig, PopuliTokenizerMode};
...
pub fn preflight_native_qlora(config: &LoraTrainingConfig) -> anyhow::Result<QloraEmbedBundle> {
    if !matches!(config.tokenizer_mode, PopuliTokenizerMode::Hf) {
    ...
    tracing::info!(
        target: "vox_populi::qlora_preflight",
