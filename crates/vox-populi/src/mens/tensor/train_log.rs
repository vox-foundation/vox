...
//!
//! **GPU visibility:** `RUST_LOG=vox_mens_gpu=info` (or `warn`) surfaces structured
//! [`tracing`] events with `event = "gpu_fallback_to_cpu"` / `"gpu_intentional_cpu"` / `"gpu_selected"`.
//!
//! General training lines use `target = "vox_mens_train"` so they appear when the CLI subscriber
...
/// Verbose training diagnostics (pair skips, mask boundaries). Enable with `RUST_LOG=debug`.
...
/// Emits `tracing::warn!` with stable `target = "vox_mens_gpu"` for log aggregation.
