...
//! | `VOX_CLOUD_IMAGE` | `ghcr.io/vox-foundation/vox-populi-cuda:latest` | Container image |
...
#[serde(rename_all = "snake_case")]
pub enum JobKind {
    /// `vox populi train` — terminates when training completes.
    Train,
    /// `vox populi serve` — persistent inference server.
    Infer,
    /// Populi agent execution — same billing model as `Infer`.
    Agent,
}
...
pub const DEFAULT_CLOUD_IMAGE: &str =
    "ghcr.io/vox-foundation/vox-populi-cuda:latest";
...
///
/// ```
/// # use vox_populi::cloud::normalize_gpu_name;
