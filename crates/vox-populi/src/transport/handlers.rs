...
pub(super) async fn dispatch_script(
    ...
    let target = if let Some(id) = &req.node_id {
        ...
        let mut candidates: Vec<_> = nodes.nodes.iter()
            .filter(|n| n.quarantined != Some(true) && !node_maintenance_blocks_new_work(crate::now_ms(), n))
            .filter(|n| {
                ...
                required.iter().all(|req_lab| n.capabilities.labels.contains(req_lab))
            })
            .collect();
    ...
    if req.is_detached {
        ...
        tokio::spawn(async move {
            ...
            match res {
                ...
                Err(e) => {
                    st_cl.dispatch_results.insert(dispatch_id_cl, DispatchResponse {
                        success: false,
                        output: String::new(),
                        error: Some(format!("populi: detached execution failed to forward: {}", e)),
        ...
        Ok(Json(DispatchResponse {
            success: true,
            output: format!("populi: detached dispatch accepted. poll for results with id: {}", dispatch_id),
            error: None,
            node_id: target.id,
            duration_ms: 0,
            exit_code: None,
...
pub(super) async fn dispatch_results_poll(
    ...
    if let Some(res) = st.dispatch_results.get(&dispatch_id) {
        ...
        Err(ResponseErr(
            StatusCode::NOT_FOUND,
            format!("populi: dispatch result for id {} not found or still in-flight", dispatch_id),
        ))
    }
}
...
pub(super) async fn execute_on_worker(
    ...
    if let Some(req_labels) = &req.required_labels {
        ...
        for req_label in req_labels {
            if !local_record.capabilities.labels.contains(req_label) {
                return Err(ResponseErr(
                    StatusCode::FORBIDDEN,
                    format!("populi capacity constraints: this node lacks the required capability label '{}'", req_label),
                ));
            }
        }
    }

    let source_bytes = base64::engine::general_purpose::STANDARD.decode(&req.source).map_err(|e| {
        ResponseErr(StatusCode::BAD_REQUEST, format!("populi: invalid base64 source: {}", e))
    })?;
    ...
    if let Some(expected_hex) = &req.source_blake3_hex {
        ...
        if &actual_hex != expected_hex {
            return Err(ResponseErr(
                StatusCode::BAD_REQUEST,
                format!("populi integrity error: bundle hash mismatch (expected {}, got {})", expected_hex, actual_hex),
            ));
        }
    }
    ...
    let bin_path = if req.is_bundle {
        ...
        std::fs::write(&tmp_file, &source_bytes).map_err(|e| {
            ResponseErr(StatusCode::INTERNAL_SERVER_ERROR, format!("populi: failed to write bundle: {}", e))
        })?;
        ...
        if !is_wasm {
            ...
            let mut perms = std::fs::metadata(&tmp_file).map(|m| m.permissions()).unwrap_or_else(|_| std::fs::Permissions::from_mode(0o755));
        ...
        let tmp_file = tmp_dir.join(file_name);
        std::fs::write(&tmp_file, &source_bytes).map_err(|e| {
            ResponseErr(StatusCode::INTERNAL_SERVER_ERROR, format!("populi: failed to write tmp file: {}", e))
        })?;
        tmp_file
    };
    ...
    match output {
        Ok(out) => {
            ...
            let output_str = String::from_utf8_lossy(&combined_stdout).to_string() + &String::from_utf8_lossy(&combined_stderr);
