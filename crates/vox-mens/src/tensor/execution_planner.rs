...
impl ExecutionPlanner {
    ...
    pub fn validate_hard_gates(&self, c: &FineTuneContract) -> anyhow::Result<()> {
        ...
        if c.adapter.method == AdapterMethod::Qlora {
            anyhow::bail!(
                "This build does not include the Candle QLoRA kernel (`candle-qlora` feature). Rebuild with `vox-mens` `train` / `candle-qlora`."
            );
        }
...
mod tests {
    ...
    use crate::tensor::training_config::{MensTokenizerMode, TrainingDeploymentTarget};
