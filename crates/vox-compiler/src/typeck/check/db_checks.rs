...
use crate::typeck::env::{BindingKind, TypeEnv};
use crate::typeck::ty::Ty;
use crate::ast::decl::TableDecl;
...
pub fn check_collection(env: &TypeEnv, diags: &mut Vec<Diagnostic>, c: &crate::ast::decl::CollectionDecl, source: &str) {
...
pub fn check_index(env: &TypeEnv, diags: &mut Vec<Diagnostic>, idx: &crate::ast::decl::IndexDecl, source: &str) {
...
pub fn check_vector_index(env: &TypeEnv, diags: &mut Vec<Diagnostic>, idx: &crate::ast::decl::VectorIndexDecl, source: &str) {
    let binding = match env.lookup(&idx.table_name) {
        Some(b) => b,
        None => {
...
pub fn check_search_index(env: &TypeEnv, diags: &mut Vec<Diagnostic>, idx: &crate::ast::decl::SearchIndexDecl, source: &str) {
    let binding = match env.lookup(&idx.table_name) {
        Some(b) => b,
        None => {
            diags.push(Diagnostic::error(
