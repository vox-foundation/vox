...
pub fn typecheck_module(module: &Module) -> Vec<Diagnostic> {
    ...
    for decl in &module.declarations {
        match decl {
            Decl::Function(f)
            | Decl::McpTool(crate::ast::decl::McpToolDecl { func: f, .. })
            | Decl::Test(crate::ast::decl::TestDecl { func: f })
            | Decl::ServerFn(crate::ast::decl::ServerFnDecl { func: f }) => {
                env.push_scope();
                env.define(
                    "db".into(),
                    Binding {
...
fn check_arguments(
    ...
    diags: &mut Vec<Diagnostic>,
    expected_args: &[Ty],
    actual_args: &[crate::ast::expr::Arg],
    span: crate::ast::span::Span,
) {
    if expected_args.len() != actual_args.len() {
        diags.push(Diagnostic {
            severity: Severity::Error,
            message: crate::typeck::diagnostics::msg_arg_count_mismatch(expected_args.len(), actual_args.len()),
            span,
            expected_type: None,
            found_type: None,
            suggestions: vec![],
...
fn infer_expr(
    ...
    match expr {
        ...
        Expr::StringInterp { parts, .. } => {
            for part in parts {
                if let crate::ast::expr::StringPart::Interpolation(expr) = part {
