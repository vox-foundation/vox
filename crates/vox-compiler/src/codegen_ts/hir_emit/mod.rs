...
pub fn emit_hir_expr(
    ...
    match expr {
        ...
        HirExpr::Jsx(el) => {
            ...
            let view = transform_hir_view_kwargs(&el.tag, &el.attributes, state_names, island_names);
...
fn transform_hir_view_kwargs(
    ...
    for attr in attrs {
        ...
        if let Some(piece) = hir_kwarg_to_class_expr(name, &attr.value, state_names, island_names) {
...
fn hir_kwarg_to_class_expr(
    ...
    match unwrap_inline_hir_block_expr(expr) {
        ...
        HirExpr::If(cond, then_stmts, else_stmts, _) => {
            ...
            let then_class = hir_kwarg_to_class_expr(kwarg, then_expr, state_names, island_names)?;
            let else_class = hir_kwarg_to_class_expr(kwarg, else_expr, state_names, island_names)?;
            let cond_str = emit_hir_expr(cond, state_names, island_names);
