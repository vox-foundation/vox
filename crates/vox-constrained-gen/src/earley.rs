...
/// Parsed grammar ready for Earley recognition.
#[derive(Debug, Clone)]
pub struct Grammar {
...
impl Grammar {
    /// Build a [`Grammar`] from the Vox EBNF string emitted by `vox-grammar-export`.
    ///
    ...
    /// notation into flat production lists (alternations become separate
    /// productions with the same LHS name).
    pub fn from_ebnf(ebnf: &str) -> Result<Self> {
        ...
        for line in ebnf.lines() {
            ...
            // Guard against `==` operators in terminal strings.
            ...
            // Split top-level alternations (` | ` outside quotes).
            ...
            for alt in split_alternatives(rhs) {
                ...
                // Accept even empty symbol lists (epsilon productions from optional constructs).
        ...
        if productions.is_empty() {
            return Err(ConstrainedGenError::GrammarError {
                reason: "EBNF produced zero productions".into(),
            });
        }
...
/// Split a production RHS on top-level `|` (not inside quotes or brackets).
...
/// Synthetic non-terminal counter for EBNF-to-BNF expansion.
...
/// Parse a sequence of EBNF symbols from a single alternative,
/// expanding `[ ... ]` (optional) and `{ ... }` (repetition) into
/// synthetic non-terminals with epsilon alternatives.
fn parse_symbols_with_expansion(alt: &str, extra_prods: &mut Vec<Production>) -> Vec<Symbol> {
    ...
    while let Some(&c) = chars.peek() {
        match c {
            ...
            '[' => {
                // Optional group: [ ... ] → synthetic NT with epsilon alternative.
                chars.next();
                ...
                if !inner_syms.is_empty() {
                    ...
                    // Two productions: one with content, one empty (epsilon).
            ...
            '{' => {
                // Repetition: { ... } → synthetic NT: content + self | epsilon.
                chars.next();
                let inner = collect_until(&mut chars, '}');
                let inner_syms = parse_symbols_with_expansion(&inner, extra_prods);
                if !inner_syms.is_empty() {
                    let name = next_synth_name("rep");
                    // Recursive: content followed by self-reference.
                    ...
                    extra_prods.push(Production {
                        name: name.clone(),
                        symbols: vec![], // epsilon (zero occurrences)
                    });
                    symbols.push(Symbol::NonTerminal(name));
                }
            }
            '(' => {
                // Grouping: ( ... ) → just inline the content.
                chars.next();
                let inner = collect_until(&mut chars, ')');
                let inner_syms = parse_symbols_with_expansion(&inner, extra_prods);
...
/// Collect characters until `closer`, respecting nested brackets.
...
mod tests {
    ...
    #[test]
    fn split_alternatives_works() {
        let alts = split_alternatives("\"true\" | \"false\"");
