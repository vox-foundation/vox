...
fn emit_voxdb_args(params: &[vox_compiler::ast::expr::Param]) -> String {
    if params.is_empty() {
        return "{}".to_string();
    }
...
pub fn generate_voxdb_handlers(module: &Module) -> String {
    ...
    for decl in &module.declarations {
        match decl {
            vox_compiler::ast::decl::Decl::Query(q) => queries.push(q),
            vox_compiler::ast::decl::Decl::Mutation(m) => mutations.push(m),
