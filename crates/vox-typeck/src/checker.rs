//! HIR type checker aligned with `vox_hir::hir` (current lowered AST).
...
impl<'a> Checker<'a> {
    ...
    fn check_arguments(&mut self, expected: &[Ty], actual: &[HirArg], span: Span) {
        if expected.len() != actual.len() {
            self.diags.push(Diagnostic::error(
                crate::diagnostics::msg_arg_count_mismatch(expected.len(), actual.len()),
                span,
                self.source,
            ));
            for arg in actual {
            ...
            HirPattern::Tuple(patterns, span) => {
                if let Ty::Tuple(tys) = &ty {
                    if tys.len() != patterns.len() {
                        self.diags.push(Diagnostic::error(
                            crate::diagnostics::msg_tuple_size_mismatch(tys.len(), patterns.len()),
                            *span,
                            self.source,
                        ));
                    }
