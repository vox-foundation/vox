...
/// Actor state storage in Codex.
pub mod actor_state;
/// Underlying Turso storage and domain operations
pub mod arca_store;
/// SHA3-512 content hashes encoded as Base32Hex
pub mod hash;
/// Source normalization before hashing
pub mod normalize;
/// Dot-separated logical namespaces
pub mod namespace;
/// Arca SQL schema manifest
pub mod schema;
...
/// Persistence for Populi QLoRA training runs.
...
pub use training_run::{TrainingRunRecord, TrainingRunStartParams};
pub use crate::arca_store::{
...
pub struct VoxDb {
    ...
    pub(crate) breaker: DbCircuitBreaker,
}

pub type CodeStore = VoxDb;
...
impl VoxDb {
    ...
    pub async fn sync_schema_from_digest(&self, digest: &SchemaDigest) -> Result<(), StoreError> {
        let migrator = AutoMigrator::new(self.connection());
    ...
    pub fn collection(&self, name: impl Into<String>) -> collection::Collection<'_> {
        collection::Collection::new(name, self.connection())
    }
    ...
    pub fn auto_migrator(&self) -> auto_migrate::AutoMigrator<'_> {
        auto_migrate::AutoMigrator::new(self.connection())
    }
    ...
    pub async fn query_all(
        ...
        let mut cursor = self.connection().query(sql, params).await?;
    ...
    ) -> Result<i64, crate::arca_store::StoreError> {
        self.store
    ...
    ) -> Result<(), crate::arca_store::StoreError> {
    ...
    pub async fn is_corpus_fresh(&self, fingerprint: &str) -> Result<bool, crate::arca_store::StoreError> {
        self.is_corpus_fresh(fingerprint).await
    }
