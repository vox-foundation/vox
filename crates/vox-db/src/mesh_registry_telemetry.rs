//! Opt-in Codex rows after mesh local registry publish (`VOX_MESH_CODEX_TELEMETRY`).
//!
...
pub async fn record_local_registry_publish_opt(
    ...
    let Ok(cfg) = DbConfig::resolve_standalone() else {
        tracing::debug!(
            target: "vox.mesh_registry_telemetry",
            "skip mesh telemetry: db config unresolved"
        );
        return;
    };
    ...
    if let Err(e) = db
        .record_mesh_control_event(repository_id, "local_registry_publish", Some(details))
        .await
    {
        tracing::debug!(
            target: "vox.mesh_registry_telemetry",
            error = %e,
            "record_mesh_control_event failed"
        );
    }
}

/// After HTTP mesh control-plane join (or failure), append one `mesh_control_event` when telemetry is on.
///
...
pub async fn record_mesh_http_join_opt(
    ...
    let Ok(cfg) = DbConfig::resolve_standalone() else {
        tracing::debug!(
            target: "vox.mesh_registry_telemetry",
            "skip mesh http join telemetry: db config unresolved"
        );
        return;
    };
    let Ok(db) = VoxDb::connect(cfg).await else {
    ...
    if let Err(e) = db
        .record_mesh_control_event(repository_id, name, Some(details))
        .await
    {
        tracing::debug!(
            target: "vox.mesh_registry_telemetry",
            error = %e,
            "record_mesh_control_event (http join) failed"
        );
    }
}
