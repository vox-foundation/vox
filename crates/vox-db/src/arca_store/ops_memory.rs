...
use crate::arca_store::types::{EmbeddingEntry, MemoryEntry, SaveMemoryParams, StoreError};
...
impl crate::VoxDb {
    ...
    pub async fn upsert_knowledge_node(
        ...
        self.conn
            .execute(
                "INSERT INTO knowledge_nodes (id, label, content, category, metadata, vcs_snapshot_id)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6)
                ...
                params![id, label, content, category, metadata, vcs_snapshot_id],
            )
            .await?;
        Ok(())
    }
            ...
            .execute(
                "INSERT INTO knowledge_edges (source_id, target_id, relation, weight, metadata)
                 ...
                 ON CONFLICT(source_id, target_id, relation) DO UPDATE SET
                    ...
                    metadata = excluded.metadata,
                    updated_at = CURRENT_TIMESTAMP",
                params![source_id, target_id, relation, weight, metadata],
            )
            .await?;
        Ok(())
    }
    ...
    pub async fn get_knowledge_neighbors(
        ...
        let mut rows = self
            .conn
            .query(
                "SELECT e.target_id, n.label, e.relation, e.weight
                 FROM knowledge_edges e
                 JOIN knowledge_nodes n ON e.target_id = n.id
                 WHERE e.source_id = ?1
                 UNION
                 SELECT e.source_id, n.label, e.relation, e.weight
                 FROM knowledge_edges e
