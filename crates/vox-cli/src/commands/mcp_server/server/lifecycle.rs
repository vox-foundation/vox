...
use vox_orchestrator::mcp_tools::{ServerState, CachedCatalog};
...
impl ServerState {
    ...
    pub async fn probe_external_orchestrator_daemon_if_configured(&self) {
        ...
        let strict_repo = strict_repo_resolved.expose()
            .is_some_and(|v| {
                let t = v.trim();
    ...
    pub fn questioning_attention_bounds(&self, session_key: &str) -> (u64, u64) {
        ...
        let max_resolved = vox_clavis::resolve_secret(vox_clavis::SecretId::VoxQuestioningMaxAttentionMs);
        let max = max_resolved.expose()
    ...
    pub fn dogfood_trace_path(&self) -> Option<PathBuf> {
        vox_clavis::resolve_secret(vox_clavis::SecretId::VoxDogfoodTracePath)
            .expose()
            .map(PathBuf::from)
            .or_else(|| {
                self.workspace_root
    ...
    pub fn dogfood_trace_path_for(&self, name: &str) -> Option<PathBuf> {
        let root = vox_repository::record_extract::vox_repository_root()?;
        let path = root
            .join("target")
            .join("dogfood")
...
/// Entry point for the native MCP server (Stdio).
pub async fn run_stdio_server_blocking() -> anyhow::Result<()> {
    ...
    crate::commands::mcp_server::populi_startup::publish_mesh_on_mcp_start(&state).await;
    ...
    let _http_gateway = crate::commands::mcp_server::http_gateway::spawn_http_gateway_if_enabled(state.clone())?;

    let server = crate::commands::mcp_server::server::VoxMcpServer::new(state);
