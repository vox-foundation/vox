...
impl ServerHandler for VoxMcpServer {
    async fn initialize(
        ...
        let tool_count = vox_orchestrator::mcp_tools::TOOL_REGISTRY.len();
    ...
    async fn list_tools(
        ...
        let mut tool_list = vox_orchestrator::mcp_tools::tool_registry();
    ...
    async fn call_tool(
        ...
        let (result_json, is_error) =
            match vox_orchestrator::mcp_tools::handle_tool_call(&self.state, &name_str, args).await {
                Ok(json) => {
