...
//! [`vox_compiler_emit::codegen_ts::scaffold`]. `@v0` uses `V0_API_KEY` when set — see `crate::v0::generate_component`.
...
///
/// `emit_scaffold`: write [`vox_compiler_emit::codegen_ts::scaffold`] files when missing (or set `VOX_WEB_EMIT_SCAFFOLD=1`).
pub async fn run(
    file: &Path,
    out_dir: &Path,
    ...
    let ts_opts = vox_compiler_emit::codegen_ts::CodegenOptions {
        tanstack_start: vox_config::VoxConfig::load().web_tanstack_start,
        target: target.clone(),
        mode: match mode {
            crate::cli_args::BuildMode::App => vox_compiler_emit::codegen_ts::emitter::BuildMode::App,
            crate::cli_args::BuildMode::Library => {
                vox_compiler_emit::codegen_ts::emitter::BuildMode::Library
            }
        },
    };
    let ts_output = vox_compiler_emit::codegen_ts::generate_with_options(&hir, ts_opts)
        .map_err(|e| anyhow::anyhow!("TypeScript codegen error: {}", e))?;

    // 6. Generate Rust (Backend)
    let rust_output = vox_compiler_emit::codegen_rust::generate(&hir, "vox_generated_app")
    ...
    if emit_scaffold || scaffold_env {
        ...
        vox_compiler_emit::codegen_ts::scaffold::write_scaffold_if_missing(project_root, "vox-app")
    ...
    if emit_ir {
        let web_ir = vox_compiler_emit::web_ir::lower::lower_hir_to_web_ir(&hir);
        let ir_json =
