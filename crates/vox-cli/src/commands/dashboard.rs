...
impl DashboardLauncher {
    async fn launch(&self, foreground: bool) -> Result<()> {
        ...
        println!(
            "Starting Vox Orchestrator on port {} with Dashboard enabled...",
            self.port
        );

        let mut child_cmd = std::process::Command::new(&self.daemon_path);
        child_cmd
            .env("VOX_MCP_HTTP_ENABLED", "1")
        ...
        if !foreground {
            std::fs::write(&pid_path, child_process.id().to_string())
                .context("write dashboard.pid")?;
        }
        ...
        if ready {
            ...
            eprintln!(
                "Warning: Daemon did not bind to port {} within 10 seconds.",
                self.port
            );
            if !foreground {
...
pub async fn run(args: DashboardArgs) -> Result<()> {
    ...
    let port = args
        .port
        .or_else(|| {
            std::env::var("VOX_DASHBOARD_PORT")
                .ok()
...
async fn stop_dashboard() -> Result<()> {
    ...
    let pid: u32 = pid_str
        .trim()
        .parse()
        .context("Invalid PID in dashboard.pid")?;
