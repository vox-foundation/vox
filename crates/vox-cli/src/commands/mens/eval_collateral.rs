...
pub fn run_collateral_damage(pre_score_path: PathBuf, post_adapter_path: PathBuf) -> Result<()> {
    ...
    if let Some(obj) = pre_json.as_object() {
        for (k, v) in obj {
            if let Some(pre) = v.as_f64() {
                // TRACKED: run inference, currently assuming pre == post for structural demo
                let post = pre;
    ...
    let config = vox_eval::CollateralDamageConfig {
    ...
    match vox_eval::eval_collateral_damage_suite(&refs, &config) {
        Ok(reports) => {
            ...
            let _ = std::fs::create_dir_all(&post_adapter_path);
            let out_json = json!({
                "status": "pass",
        ...
        Err(failing_report) => {
            ...
            eprintln!(
                "Benchmark '{}' degraded by {:.1}% (limit {:.1}%)",
