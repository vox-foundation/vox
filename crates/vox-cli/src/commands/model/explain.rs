...
pub async fn run(args: ExplainArgs) -> anyhow::Result<()> {
    ...
    println!("{} Model Selection for task: \"{}\"", " EXPLAIN ".on_blue().white().bold(), description.italic());
    println!("Category: {:?}, Complexity: {}", category, complexity);
    println!("---");

    let strength = vox_orchestrator::models::task_category_strength(category);
    let candidates = registry.explain_selection(category, strength, vox_orchestrator::config::CostPreference::Performance);
    ...
    println!("{} Top Candidates (sorted by priority score):", " RANK ".on_green().black().bold());
    ...
    if let Ok(Some(tid)) = db.get_last_interaction_trace_id(&category.to_string()).await {
