...
pub async fn run(_args: DiscoverArgs) -> anyhow::Result<()> {
    println!("{} Discovering models...", " INFO ".on_blue().white().bold());
    ...
    if let Ok(models) = or_catalog.refresh().await {
        println!("  ✅ Discovered {} models from OpenRouter", models.len().green());
    ...
    if let Ok(models) = ollama_catalog.refresh().await {
        println!("  ✅ Discovered {} models from local Ollama", models.len().green());
        all_models.extend(models);
    }
    ...
    if let Ok(models) = hf_catalog.refresh().await {
        println!("  ✅ Discovered {} models from Hugging Face", models.len().green());
        all_models.extend(models);
    }
    ...
    if let Ok(models) = mesh_catalog.refresh().await {
        println!("  ✅ Discovered {} models from Populi Mesh", models.len().green());
        all_models.extend(models);
    }

    println!("\n✅ Total discovered models: {}", all_models.len().green().bold());
