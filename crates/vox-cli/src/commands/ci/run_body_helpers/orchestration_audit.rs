...
pub async fn run_ssot_audit(_root: &Path) -> Result<()> {
    ...
    use vox_orchestrator::types::{AgentTask, TaskCategory, TaskPriority, TaskId};
    ...
    let scores = match db.get_model_scoreboard(30).await {
        Ok(s) => s,
        Err(e) => {
            println!("  [WARN] Failed to fetch model scoreboard: {}. Continuing with static config only.", e);
            vec![]
        }
    };
    
    let score_map = scores.into_iter().map(|s| (s.model_id.clone(), s.into())).collect();
    registry.inject_scoreboard(score_map);

    println!("  Loaded scoreboard for {} models.", registry.scoreboard_len());
    ...
    for cat in test_categories {
        let mut task = AgentTask::new(TaskId(0), format!("Audit task for {:?}", cat), TaskPriority::Normal, vec![]);
        ...
        if let Some(m) = econ_model {
            ...
            let cost_basis = registry.get_score(&m.id)
                .and_then(|s| s.cost_per_success_usd)
                .unwrap_or(m.cost_per_1k);
            let est_cost = (est_tokens as f64 / 1000.0) * cost_basis;

            if est_cost > 0.05 {
                 println!("  [FAIL] Economy Routing ({}): selected {} at estimated cost ${:.4}, exceeding budget!", cat, m.id, est_cost);
                 violations += 1;
            } else {
                 println!("  [OK] Economy Routing ({}): selected {} at estimated cost ${:.4}", cat, m.id, est_cost);
            }
        } else {
            // It's possible no model fits a tight budget, but we should have free models as fallbacks
            println!("  [WARN] Economy Routing ({}): no model selected for tight budget ($0.05). Ensure free models are available.", cat);
        }
    }

    if violations > 0 {
        return Err(anyhow!("Orchestration SSOT Audit failed with {} violations.", violations));
    }
