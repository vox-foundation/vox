...
pub async fn run(
    ...
    if !dry_run {
        ...
        std::fs::create_dir_all("populi/data/mix_sources")?;
    }
    ...
    for stage in planned_stages {
        ...
        match stage {
            PipelineStage::Generate => {
                if !dry_run {
                    crate::commands::corpus::run(crate::commands::corpus::CorpusAction::Generate {
                        output: PathBuf::from("populi/data/synthetic.jsonl"),
                        force_regen: false,
                        dry_run: false,
                    })
                    .await?;
                ...
                if !dry_run {
                    ...
                    if crates_dir.is_dir() {
                        let _ = crate::commands::corpus::run(crate::commands::corpus::CorpusAction::ExtractRs {
                            dir: crates_dir,
                            output: PathBuf::from("populi/data/mix_sources/rust_source.jsonl"),
                        })
                        .await;
                    }
                    ...
                    if docs_dir.is_dir() {
                        let _ = crate::commands::corpus::run(crate::commands::corpus::CorpusAction::ExtractDocs {
                            dir: docs_dir,
                            output: PathBuf::from("populi/data/mix_sources/docs.jsonl"),
                        })
                        .await;
                    }
                }
            ...
            PipelineStage::Replay => {
                if !dry_run {
                    crate::commands::corpus::run(crate::commands::corpus::CorpusAction::Replay {
                        ...
                        output: PathBuf::from("populi/data/mix_sources/autofeedback.jsonl"),
                        limit: 1000,
                    })
                    .await?;
                }
            ...
            PipelineStage::Mix => {
                if !dry_run {
                    let mix_config = PathBuf::from("populi/config/mix.yaml");
                ...
                if !dry_run {
                    #[cfg(feature = "gpu")]
                    {
                        ...
                        crate::commands::populi::train::run_train(
                            crate::commands::populi::PopuliTrainBackendCli::Qlora.into(),
                            Some(target_model),
                            device,
                            data_dir.clone(),
                            output_dir.clone(),
                            ...
                            crate::commands::populi::PopuliTokenizerCli::Hf.into(),
