//! `vox populi train` — native LoRA training worker.
...
/// Spawn `vox populi train` in a background process with stdout/stderr redirected to a log file.
...
pub fn run_train(
    ...
    deployment_target: vox_populi::TrainingDeploymentTarget,
    ...
    let device_kind =
        vox_populi::normalize_device(&device).map_err(|e| anyhow::anyhow!("{}", e))?;
    vox_populi::apply_backend_env(device_kind);

    if matches!(
        deployment_target,
        vox_populi::TrainingDeploymentTarget::MobileEdge
    ) && !matches!(device_kind, vox_populi::DeviceKind::Cpu)
    {
        anyhow::bail!(vox_populi::operator_messages::MOBILE_EDGE_REQUIRES_CPU_DEVICE);
    }

    if matches!(train_backend, vox_populi::PopuliTrainBackend::CandleQlora)
        && tokenizer_mode != vox_populi::PopuliTokenizerMode::Hf
    {
        anyhow::bail!(vox_populi::operator_messages::QLORA_REQUIRES_HF_TOKENIZER);
    }
    ...
    if matches!(train_backend, vox_populi::PopuliTrainBackend::CandleQlora) {
        if matches!(device_kind, vox_populi::DeviceKind::Cuda) {
            #[cfg(not(feature = "populi-candle-cuda"))]
            anyhow::bail!(
                "`--device cuda` for Candle QLoRA requires a CUDA-enabled build.\n\
                 Rebuild: `cargo build -p vox-cli --features gpu,populi-candle-cuda` (or `cargo vox-cuda-release`).\n\
                 On Windows use a VS Developer shell so `nvcc` can find MSVC."
            );
        }
        #[cfg(target_os = "macos")]
        if matches!(device_kind, vox_populi::DeviceKind::Metal) {
            #[cfg(not(feature = "populi-candle-metal"))]
            anyhow::bail!(
                "`--device metal` for Candle QLoRA requires `populi-candle-metal`.\n\
                 Rebuild: `cargo build -p vox-cli --features gpu,populi-candle-metal`."
            );
        }
    }
    ...
    tracing::debug!(
        ...
        "vox populi train entry (backend + tokenizer SSOT)"
    );
    ...
    let gpu_info = vox_populi::probe_gpu();
    let device_profile =
        vox_populi::DeviceProfile::from_gpu_info(&gpu_info.model_name, gpu_info.vram_mb);
    let cli_overrides = vox_populi::CliOverrides {
        rank,
        alpha,
        seq_len,
        batch_size,
    ...
    let preview_profile = vox_populi::resolve_effective_profile(
        preset.as_deref(),
    ...
    eprintln!("{}", "╔══════════════════════════════════════════╗".cyan());
    eprintln!("{}", "║   Vox Populi — Native LoRA Training     ║".cyan());
    eprintln!("{}", "╚══════════════════════════════════════════╝".cyan());
    eprintln!();
    eprintln!(
        "  Model:       {}",
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        let mix_config_path: Option<std::path::PathBuf> = workspace_root
            .as_ref()
            .map(|r| r.join("populi/config/mix.yaml"));
        ...
        if let Some(ref repo_id) = model {
            ...
            std::thread::spawn(move || {
                ...
                let result = rt.block_on(vox_populi::hub::download_model(&repo_id));
            ...
            match download_result {
                Ok(files) if files.is_safetensors() => {
                    ...
                    if let Ok(arch) =
                        vox_populi::tensor::hf_load::detect_hf_architecture(&files.config)
                    {
                        eprintln!("  {} Architecture: {:?}", "📐".cyan(), arch);
                        let cfg = vox_populi::tensor::hf_load::config_dims_for_architecture(
                            &files.config,
                            arch,
                        )
                        ...
                        let est_mb =
                            if matches!(train_backend, vox_populi::PopuliTrainBackend::CandleQlora)
                            {
                                vox_populi::estimate_training_vram_mb_qlora(
                                    cfg.n_embd,
                                    cfg.n_head,
                                    cfg.n_layer,
                                    cfg.vocab_size,
                                ...
                                vox_populi::estimate_training_vram_mb(
                                    cfg.n_embd,
                                    cfg.n_head,
                                    cfg.n_layer,
                                    cfg.vocab_size,
        ...
        let device_profile_str = if device_kind == vox_populi::DeviceKind::Cpu {
            "cpu".to_string()
        } else {
            gpu_info.model_name.clone()
        };
        let config = vox_populi::LoraTrainingConfig {
            base_model: model,
            base_model_paths,
            tokenizer_path,
    ...
    #[cfg(not(feature = "gpu"))]
    {
        ...
        eprintln!();
        eprintln!(
            "  Canonical QLoRA (when `gpu` is enabled): `vox populi train --backend qlora …`"
        );
        eprintln!("  See docs/src/architecture/populi-training-ssot.md");
        Ok(())
    }
}
