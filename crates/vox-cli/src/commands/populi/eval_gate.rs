//! `vox populi eval-gate` — check training/eval metrics against policy thresholds.
//!
...
/// [`check_run`] / `vox populi eval-gate` agree with `eval_local` policy thresholds.
#[cfg(all(feature = "populi-dei", feature = "gpu"))]
...
/// Load policy from path (default: populi/config/eval-gates.yaml).
        ...
        if let Some(ref eval) = eval_json {
            ...
            for (ctx_name, gate) in &policy.per_context {
                results.push(GateResult {
                    name: format!("per_context[{}]", ctx_name),
                    passed: false,
                    message: "eval_results.json not found — run `vox populi eval` first"
                        .to_string(),
                    block: gate.block,
                });
            }
...
pub fn run_eval_gate(
    ...
    let policy_path =
        policy_path.unwrap_or_else(|| std::path::PathBuf::from("populi/config/eval-gates.yaml"));
    ...
    eprintln!("{}", "╔══════════════════════════════════════════╗".cyan());
    eprintln!("{}", "║   Vox Populi — Eval Gate Check           ║".cyan());
    eprintln!("{}", "╚══════════════════════════════════════════╝".cyan());
