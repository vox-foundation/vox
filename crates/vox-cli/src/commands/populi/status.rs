//! `vox populi status` — training run status from telemetry.
...
pub async fn run_status(
    ...
    let base = run_dir.unwrap_or_else(|| PathBuf::from("populi/runs/latest"));
    ...
    if !telemetry_path.exists() && run_state.is_none() {
        ...
        eprintln!("  Run `vox populi train` to generate training metrics.");
        return Ok(());
    }
...
async fn display_quotas(_as_json: bool) -> Result<()> {
    anyhow::bail!(
        "BYOK quota rollups previously depended on the excluded `vox-dei` crate (usage tracker). \
         ...
         Use `vox populi status` without `--quotas`, or check limits in your provider console."
    );
}
