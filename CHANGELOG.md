...
- **`vox-cli`**: default features are **`populi-base` only** (no **`gpu`**). Native Populi train / probe / merge / eval-local require **`cargo build -p vox-cli --features gpu`**. **`vox-populi`** binary (prepends `populi` subcommand). **`vox-codex`** removed as a **`vox-cli`** dependency (`codex` / `stub-check` use **`vox-db`**); OS keyring helpers live in **`vox_db::secrets`** (**`vox-codex`** still re-exports for other crates). **`vox-corpus`** and **`vox-runtime`** are **always** linked so grammar / training JSONL paths work even with **`--no-default-features`** (`populi-base` remains the **command-surface** gate).
...
- **`vox ci build-timings`** — wall-clock **`cargo check`** for default `vox-cli`, GPU+stub lane, optional CUDA lane (`--json` supported). **`--crates`** adds `vox-cli --no-default-features`, `vox-db`, `vox-oratio`, `vox-populi --features train`, and **`vox-cli --features populi-oratio`** lanes. Optional soft budgets: `docs/ci/build-timings/budgets.json` with **`VOX_BUILD_TIMINGS_BUDGET_WARN=1`** / **`VOX_BUILD_TIMINGS_BUDGET_FAIL=1`**. GitHub CI runs **`build-timings --crates`** in place of the standalone GPU/stub check step.
- **`populi-oratio`** feature — Oratio / `vox-oratio` is no longer pulled by default **`populi-base`**; enable **`--features populi-oratio`** for **`vox populi oratio`** / **`vox ai oratio`**. CI feature matrix includes a **`populi-oratio`** compile lane.
...
- **CLI / Populi**: `vox populi pipeline` — dogfood corpus → eval → optional native train (replaces PS1 orchestration); `scripts/run_populi_pipeline.ps1` is a thin delegate.
...
- **GitLab**: `vox-ci-guards` aligned with GitHub (`toestub-scoped`, `cuda-features`, `populi-gate --profile ci_full`); `NATIVE_TRAIN=false` rejected on `ml-train`.
- **CLI / CI**: `vox ci` guard commands (manifest, SSOT checks, doc-inventory, workflow allowlist, Populi `gates.yaml` runner, TOESTUB scoped, CUDA probes); crate `vox-doc-inventory`.
- **Scripts**: `ci_full` Populi profile; GitHub `ci.yml` cut over to `cargo run -p vox-cli -- ci …`; thin shell/ps1 delegates; `docs/agents/script-registry.json` + baseline metrics.
