...
|------|----------------|-------------------------|
| **MCP HTTP** (this repo’s `Dockerfile`) | Build from repo root; optional `VOX_CLI_FEATURES` for mesh | `CMD ["vox","mcp"]` — **3000** |
...
See [`docker-compose.yml`](docker-compose.yml). For **mesh** alongside other services, merge env from [`docker/vox-compose-mesh-environment.block.yaml`](../../docker/vox-compose-mesh-environment.block.yaml) and follow [mesh SSOT](../../docs/src/architecture/mesh-ssot.md).
