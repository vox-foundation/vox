...
METAL_FUNC IndexT get_strided_idx_fallback(
    ...
    IndexT strided_i = 0;
    // Vox fix: peel the innermost (num_dims - D) dims first, then let
    // strided_indexer<D> handle dims[0..D). Upstream indexed dims[num_dims-1-d],
    // i.e. the *outer* dims, so every rank>4 strided reduce read wrong elements.
