...
static inline void
aegis128x4_absorb_rate(const uint8_t *const src, aes_block_t *const state)
{
    aes_block_t msg0, msg1;
...
static inline void
aegis128x4_squeeze_keystream(uint8_t *const dst, aes_block_t *const state)
{
...
stream(uint8_t *out, size_t len, const uint8_t *npub, const uint8_t *k)
{
    ...
    for (i = 0; i + RATE <= len; i += RATE) {
        aegis128x4_enc(out + i, src, state);
    }
    if (len % RATE) {
        aegis128x4_enc(dst, src, state);
...
static int
state_encrypt_update(aegis128x4_state *st_, uint8_t *c, const uint8_t *m, size_t mlen)
{
    aegis_blocks             blocks;
    _aegis128x4_state *const st =
        (_aegis128x4_state *) ((((uintptr_t) &st_->opaque) + (ALIGNMENT - 1)) &
...
static int
state_encrypt_final(aegis128x4_state *st_, uint8_t *mac, size_t maclen)
{
    aegis_blocks             blocks;
    _aegis128x4_state *const st =
        (_aegis128x4_state *) ((((uintptr_t) &st_->opaque) + (ALIGNMENT - 1)) &
    ...
    if (st->pos != 0) {
        ...
        aegis128x4_absorb_rate(tmp, blocks);
    }
...
static int
state_decrypt_final(aegis128x4_state *st_, const uint8_t *mac, size_t maclen)
{
    aegis_blocks             blocks;
    CRYPTO_ALIGN(16) uint8_t computed_mac[32];
    _aegis128x4_state *const st =
