...
- [ ] New event `type` has **`base_reward`** and **`process_event_rewards`** companion/quest/counter behavior _or_ is documented as policy-only in a living doc or commit message (the archived event-kind matrix in `docs/src/archive/research-2026-q1/` should not be used as a reference — add new event types to a current doc).
